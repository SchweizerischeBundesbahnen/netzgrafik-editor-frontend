...
- `"one-way"`: trainrun represents `A -> B -> C`
- `"one-way"`: trainrun represents `A <- B <- C` (same value of `TrainrunDirection` attribute, but the trainrun (so the nodes path) is inverted)
