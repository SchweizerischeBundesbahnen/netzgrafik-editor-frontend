...
export class KnotenAuslastungDataPreparation {
  ...
  addTrainrunSectionToMatrix(trainrunSectionObject: TrainrunSection, node: Node, isTargetNode: boolean) {
    for (let directionLoop = 0; directionLoop < 2; directionLoop += 1) {
      const haltezeit = node.getTrainrunCategoryHaltezeit();
      const delta = Math.floor(haltezeit[trainrunSectionObject.getTrainrun().getTrainrunCategory().fachCategory].haltezeit);
      let arrivalTime = isTargetNode ? trainrunSectionObject.getTargetArrivalConsecutiveTime() :
        trainrunSectionObject.getSourceArrivalConsecutiveTime();
      let targetNode = isTargetNode ? trainrunSectionObject.getTargetNode() : trainrunSectionObject.getSourceNode();
      if (directionLoop === 0) {
        arrivalTime = (isTargetNode ? trainrunSectionObject.getTargetDepartureConsecutiveTime() :
          trainrunSectionObject.getSourceDepartureConsecutiveTime()) - delta + 60;
        targetNode = isTargetNode ? trainrunSectionObject.getSourceNode() : trainrunSectionObject.getTargetNode();
      }
      ...
      for (let freqLoop = 0; freqLoop < 60; freqLoop += freq) {
        ...
        for (let uLoop = freqArrivalTime; uLoop <= freqDeparturetime; uLoop += 1) {
          this.matrix[uLoop % 60][freeIdx] = trainrunSectionObject.getTrainrunId();
        }
        this.nbrUsedOfTrackFound = Math.max(this.nbrUsedOfTrackFound, freeIdx);
        this.nodeDatas.push(
          {
            trainrunSection: trainrunSectionObject,
            name: KnotenAuslastungDataPreparation.getTrainrunLabel(trainrunSectionObject),
  ...
  getRealStopTime(trainrunSection1: TrainrunSection, trainrunSection2: TrainrunSection, node: Node): number {
    let stopTime = 0;
    if (trainrunSection1.getSourceNodeId() === node.getId()) {
      if (trainrunSection2.getSourceNodeId() === node.getId()) {
        stopTime = trainrunSection2.getSourceDepartureConsecutiveTime() - trainrunSection1.getSourceArrivalConsecutiveTime();
      } else {
        stopTime = trainrunSection2.getTargetDepartureConsecutiveTime() - trainrunSection1.getSourceArrivalConsecutiveTime();
      }
    } else {
      if (trainrunSection2.getSourceNodeId() === node.getId()) {
        stopTime = trainrunSection2.getSourceDepartureConsecutiveTime() - trainrunSection1.getTargetArrivalConsecutiveTime();
      } else {
        stopTime = trainrunSection2.getTargetDepartureConsecutiveTime() - trainrunSection1.getTargetArrivalConsecutiveTime();
      }
    }
  ...
  computeAuslastungsMatrix(node: Node) {
    ...
    this.matrix = new Array(KnotenAuslastungDataPreparation.MAX_NR_MINUTES)
      .fill(undefined)
      .map(() => new Array(KnotenAuslastungDataPreparation.MAX_NR_TRACKS)
        .fill(undefined));
    ...
    sortedTransitions.forEach((transition: Transition) => {
      const trainrunSection11 = node.getPort(transition.getPortId1()).getTrainrunSection();
      const trainrunSection12 = node.getPort(transition.getPortId2()).getTrainrunSection();
      this.addTransitionsToMatrix(trainrunSection11, trainrunSection12, node.getId());
      const trainrunSection21 = node.getPort(transition.getPortId2()).getTrainrunSection();
      const trainrunSection22 = node.getPort(transition.getPortId1()).getTrainrunSection();
      this.addTransitionsToMatrix(trainrunSection21, trainrunSection22, node.getId());
    });

    this.trainrunSectionService.getTrainrunSections()
      .forEach((ts: TrainrunSection) => {
        if (ts.getTargetNodeId() === node.getId() &&
          node.getNextTrainrunSection(ts) === undefined) {
          this.addTrainrunSectionToMatrix(ts, node, true);
        }
        if (ts.getSourceNodeId() === node.getId() &&
          node.getNextTrainrunSection(ts) === undefined) {
        ...
        this.resourceDatas.push(
          {
            ...
            capacityLimitReached: trackLoop >= this.resourceService.getResource(node.getResourceId()).getCapacity()
          });
      }
    }
  }
  ...
  private addTransitionsToMatrix(trainrunSection1: TrainrunSection, trainrunSection2: TrainrunSection, nodeId: number) {
    ...
    for (let freqLoop = 0; freqLoop < KnotenAuslastungDataPreparation.MAX_NR_MINUTES; freqLoop += freq) {
      const freqArrivalTime = Math.round(arrivalTime + freqLoop) % 60;
      const freqDeparturetime = Math.round(freqArrivalTime + delta);
