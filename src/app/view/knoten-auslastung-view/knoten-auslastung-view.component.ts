...
export class KnotenAuslastungViewComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private uiInteractionService: UiInteractionService,
    ...
    this.knotenAuslastungDataPreparation = new KnotenAuslastungDataPreparation(trainrunService, resourceService, trainrunSectionService);
  }

  static isMuted(trainrunSection: TrainrunSection, selectedTrainrun: Trainrun, connectedTrainIds: number[]): boolean {
  ...
  static createTrainrunSectionFrequencyClassAttribute(trainrunSection: TrainrunSection,
                                                      ...
                                                      connectedTrainIds: number[]): string {
    let classAttribute =
      StaticDomTags.makeClassTag(StaticDomTags.FREQ_LINE_PATTERN, trainrunSection.getFrequencyLinePatternRef()) +
      StaticDomTags.makeClassTag(StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.TAG_COLOR_REF,
    ...
    if (!KnotenAuslastungViewComponent.isMuted(trainrunSection, selectedTrainrun, connectedTrainIds)) {
  ...
  ngAfterViewInit(): void {
    this.svgDrawingContext =
      d3.select('#knotenAuslastungContainer')
  ...
  private subscribeViewToServices() {
    this.uiInteractionService.updateNodeStammdatenWindow.pipe(takeUntil(this.destroyed)).subscribe(() => {
      this.update();
    });
  ...
  private update() {
    ...
    if (selectedTrainrun !== null) {
      connectedTrainIds = this.trainrunService.getConnectedTrainrunIdsFirstOrder(selectedTrainrun.getId());
    }
    ...
    const arc = d3.arc()
      ...
      .innerRadius(d => ((1 + (d.innerRadius + 0.05)) / (2 + Math.max(nbrUsedOfTrackFound, 0))) * pixelRadius)
      .outerRadius(d => ((1 + (d.outerRadius + 0.95)) / (2 + Math.max(nbrUsedOfTrackFound, 0))) * pixelRadius);

    this.svgDrawingContext.selectAll('g.KnotenAuslastungResourceGroup').remove();
    const rootResourceGroup = this.svgDrawingContext.selectAll('g.KnotenAuslastungResourceGroup')
      .data(resourceDatas);
    rootResourceGroup.enter()
    ...
    const rootDataGroup = this.svgDrawingContext.selectAll(StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP_G)
      .data(nodeDatas);
    rootDataGroup.enter()
      .append(StaticDomTags.GROUP_SVG)
      ...
      .attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')')
      .append('path')
      .attr('class', (d) => StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP +
        KnotenAuslastungViewComponent.createTrainrunSectionFrequencyClassAttribute(d.trainrunSection,
          selectedTrainrun, connectedTrainIds))
      .attr('d', arc)
      .on('mousedown', (d) => this.trainrunService.setTrainrunAsSelected(d.trainrunSection.getTrainrunId()))
      .append('title')
      .html((d) => d.tooltip);

    const groupText = rootDataGroup.enter()
      .append(StaticDomTags.GROUP_SVG)
      .attr('class', StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP)
      .attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')')
      .append('text')
      ...
      .attr('transform', (d) => {
        ...
        return 'translate(' + arc.centroid(d)[0] +
          ',' + arc.centroid(d)[1] + ') rotate(-90) rotate(' + angle + ')';
      })
      .attr('dy', '.35em')
      .attr('x', 0)
      .attr('y', 0)
    ...
    const nbrOfTrackGroup = this.svgDrawingContext.selectAll('g.KnotenAuslastungNbrTrackGroup')
      .data([nbrUsedOfTrackFound + 1]);
    nbrOfTrackGroup.enter()
      .append(StaticDomTags.GROUP_SVG)
      ...
      .html((d) => '' +
        Math.round(
          100 * d / this.resourceService.getResource(selectedNode.getResourceId()).getCapacity()
        ) + '%'
      );

    this.svgDrawingContext.selectAll('g.KnotenAuslastungTimeGroup').remove();
    const timeGroup = this.svgDrawingContext.selectAll('g.KnotenAuslastungTimeGroup')
