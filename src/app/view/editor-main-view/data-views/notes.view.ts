...
export class NotesView {
  ...
  static convertText(strToConvert: string): string {
    return strToConvert.trim()
      ...
      .split('</li>').join('')
      .split('<li><p>').join('<p> - ')
      .split('<br>').join('<tspan x="' + (NOTE_TEXT_LEFT_SPACING - 4) + '" dy="' + TEXT_SIZE + '">&nbsp;</tspan>')
      .split('<p>').join('<tspan x="' + NOTE_TEXT_LEFT_SPACING + '" dy="' + 1.5 * TEXT_SIZE + '">')
      .split('</p>').join('&nbsp;</tspan>')
      .split('<em>').join('<tspan style="font-style: italic">')
      .split('</em>').join('&nbsp;</tspan>')
      .split('<strong>').join('<tspan style="font-weight: bold">')
      .split('</strong>').join('&nbsp;</tspan>')
      .split('<span').join('<tspan')
      .split('</span').join('</tspan')
      .split('style="color:').join('style="fill:');
  }

  static extractTextBasedHeight(n: Note): number {
    return Math.max(n.getText().split('<br>').join('<p>')
      .split('<p>', 9999).length * 1.5 * TEXT_SIZE + NOTE_TEXT_AREA_HEIGHT + 16, n.getHeight());
  }
  ...
  createViewNoteDataObjects(nodes: Note[]): NoteViewObject[] {
    ...
    nodes.forEach(
      (n: Note) => {
        viewNoteDataObjects.push(
          new NoteViewObject(this.editorView, n));
      });
  ...
  displayNotes(inputNotes: Note[]) {
    ...
    const groupEnter = groupEnter2
      ...
      .attr('transform', (n: NoteViewObject) => 'translate(' + n.note.getPositionX() + ',' + n.note.getPositionY() + ')');

    groupEnter.append(StaticDomTags.NOTE_HOVER_ROOT_SVG)
    ...
    groupEnter.append(StaticDomTags.NOTE_ROOT_SVG)
      ...
      .attr('x', -20)
      .attr('y', -20)
      .classed(StaticDomTags.TAG_MULTI_SELECTED, (n: NoteViewObject) =>
    ...
    groupEnter.append(StaticDomTags.NOTE_TITELAREA_SVG)
    ...
    groupEnter.append(StaticDomTags.NOTE_TEXTAREA_SVG)
      .attr('class', StaticDomTags.NOTE_TEXTAREA_CLASS)
      .attr(StaticDomTags.NOTE_ID, (n: NoteViewObject) => n.note.getId())
      .attr('width', (n: NoteViewObject) => NotesView.extractTextBasedWidth(n.note))
      .attr('height', (n: NoteViewObject) => NotesView.extractTextBasedHeight(n.note) - NOTE_TEXT_AREA_HEIGHT)
      .attr('x', 0)
    ...
    groupEnter.append(StaticDomTags.NOTE_TITELAREA_TEXT_SVG)
    ...
    groupEnter.append(StaticDomTags.NOTE_TEXT_SVG)
    ...
    groupEnter.append(StaticDomTags.NOTE_HOVER_DRAG_AREA_BACKGROUND_SVG)
    ...
    groupEnter.append(StaticDomTags.NOTE_HOVER_DRAG_AREA_SVG)
  ...
  onNoteMouseover(note: Note, domObj: any) {
    ...
    if ((this.editorView.editorMode === EditorMode.TopologyEditing) ||
  ...
  onNoteMouseoverDragButton(note: Note, domObj: any) {
    ...
    d3.selectAll(StaticDomTags.NOTE_HOVER_DRAG_AREA_DOM_REF)
      .filter((n: NoteViewObject) => n.note.getId() === note.getId()).classed(StaticDomTags.TAG_HOVER, true);
  }

  onNoteMouseoutDragButton(note: Note, domObj: any) {
    d3.selectAll(StaticDomTags.NOTE_HOVER_DRAG_AREA_DOM_REF)
      .filter((n: NoteViewObject) => n.note.getId() === note.getId()).classed(StaticDomTags.TAG_HOVER, false);
    this.onNoteMouseout(note, domObj);
  }

  hoverNote(note: Note, domObj: any) {
    d3.selectAll(StaticDomTags.NOTE_HOVER_DRAG_AREA_DOM_REF)
      .filter((n: NoteViewObject) => n.note.getId() === note.getId()).classed(StaticDomTags.TAG_MUTED, true);
  ...
  unhoverNote(note: Note, domObj: any) {
    d3.selectAll(StaticDomTags.NOTE_HOVER_DRAG_AREA_DOM_REF)
      .filter((n: NoteViewObject) => n.note.getId() === note.getId()).classed(StaticDomTags.TAG_MUTED, false);
    d3.selectAll(StaticDomTags.NOTE_ROOT_DOM_REF)
      .filter((n: NoteViewObject) => n.note.getId() === note.getId()).classed(StaticDomTags.TAG_HOVER, false);
  }
