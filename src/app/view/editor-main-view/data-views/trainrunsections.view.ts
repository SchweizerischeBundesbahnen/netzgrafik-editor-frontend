...
export class TrainrunSectionsView {
  ...
  static extractTravelTime(
    ...
    isBackwardTravel = TrainrunsectionHelper.isTargetLeftOrTop(trainrunSection)
      ? !isBackwardTravel
      : isBackwardTravel;
  ...
  getHiddenTagForTime(
    ...
    switch (textElement) {
      ...
      case TrainrunSectionText.TrainrunSectionBackwardTravelTime:
        if (trainrunSection.getTrainrun().getTrainrunDirection() === TrainrunDirection.ONE_WAY) {
          return true;
        }
        // TODO: should we keep travelTimeEqualBackwardTravelTime() filter?
        if (trainrunSection.isSymmetric() || trainrunSection.travelTimeEqualBackwardTravelTime()) {
          return true;
        }
        if (!this.editorView.isFilterTravelTimeEnabled()) {
          return true;
  ...
  translateAndRotateArrow(
    ...
    arrow: "DIRECTION_ARROW" | "SYMMETRY_ARROW",
  ) {
    ...
    if (isTargetRightOrBottom) {
      arrowOffset = arrow === "DIRECTION_ARROW" ? [-44, 20] : [-32, 32];
    } else {
      arrowOffset = arrow === "DIRECTION_ARROW" ? [44, -20] : [32, -32];
    }
    let x, y: number;
  ...
  createSymmetryArrows(
    ...
    (["BEGINNING_ARROW", "ENDING_ARROW"] as const).forEach((arrowType) => {
      groupLinesEnter
        .append(StaticDomTags.EDGE_LINE_ARROW_SVG)
        .attr(StaticDomTags.TAG_HIDDEN, () =>
          !this.editorView.isFilterTrainrunSectionSymmetryArrowsEnabled() ? "" : null,
        )
        .attr("d", (d: TrainrunSectionViewObject) => {
          if (arrowType === "BEGINNING_ARROW") {
            return d.trainrunSection.isSourceSymmetricOrTimesEqual()
              ? ""
              : "M-1,-8 V0 H-10 L1,8 V0 H10 Z";
          }
          else {
            return d.trainrunSection.isTargetSymmetricOrTimesEqual()
              ? ""
              : "M-1,-8 V0 H-10 L1,8 V0 H10 Z";
          }
        })
    ...
    inputTrainrunSections.forEach((d: TrainrunSection) => {
      viewTrainrunSectionDataObjects.push(
        new TrainrunSectionViewObject(
          ...
          !this.editorView.isFilterTrainrunSectionSymmetryArrowsEnabled(),
        ),
      );
    });
