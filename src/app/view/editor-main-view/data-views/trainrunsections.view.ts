...
export class TrainrunSectionsView {
  ...
  static translateAndRotateText(trainrunSection: TrainrunSection, trainrunSectionText: TrainrunSectionText) {
  ...
  static isMuted(trainrunSection: TrainrunSection, selectedTrainrun: Trainrun, connectedTrainIds: any): boolean {
    if (connectedTrainIds !== undefined && connectedTrainIds.indexOf(trainrunSection.getTrainrunId()) !== -1) {
      return false;
    }
  ...
  static createTrainrunSectionFrequencyClassAttribute(trainrunSection: TrainrunSection,
                                                      selectedTrainrun: Trainrun,
                                                      connectedTrainIds: any): string {
    let classAttribute =
      StaticDomTags.makeClassTag(StaticDomTags.FREQ_LINE_PATTERN, trainrunSection.getFrequencyLinePatternRef()) +
      StaticDomTags.makeClassTag(StaticDomTags.TAG_COLOR_REF, trainrunSection.getTrainrun().getCategoryColorRef()) +
      StaticDomTags.makeClassTag(StaticDomTags.TAG_LINEPATTERN_REF, trainrunSection.getTrainrun().getTimeCategoryLinePatternRef());

    if (TrainrunSectionsView.isMuted(trainrunSection, selectedTrainrun, connectedTrainIds) === true) {
  ...
  static getPosition(trainrunSection: TrainrunSection, atSource: boolean): Vec2D {
    return atSource ? trainrunSection.getPositionAtSourceNode() : trainrunSection.getPositionAtTargetNode();
  }
  ...
  static getFormattedDisplayText(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): string {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
  ...
  static getFormattedDisplayTextWidth(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): number {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
  ...
  static getAdditionTextCloseToNodePositioningValue(trainrunSection: TrainrunSection, atSource: boolean): string {
    const path = trainrunSection.getPath();
    let pos: Vec2D;
    if (atSource) {
  ...
  static getAdditionPositioningValue(trainrunSection: TrainrunSection, textElement: TrainrunSectionText) {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
      case TrainrunSectionText.SourceArrival:
      case TrainrunSectionText.TargetDeparture:
  ...
  static getTrainrunSectionTimeElementClass(trainrunSection: TrainrunSection, textElement: TrainrunSectionText,
                                            selectedTrainrun: Trainrun, connectedTrainIds: any): string | undefined {
    ...
    const timeTag = StaticDomTags.EDGE_LINE_TEXT_CLASS + ' ' + TrainrunSectionText[textElement] +
      ' ' + (
        colorRef === undefined ?
          StaticDomTags.makeClassTag(StaticDomTags.TAG_COLOR_REF, trainrunSection.getTrainrun().getCategoryColorRef()) :
          colorRef
    ...
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
        return timeTag + TrainrunSectionsView.getTrainrunSectionTimeElementOddOffsetTag(
          trainrunSection.getSourceDepartureConsecutiveTime(), trainrunSection.getTrainrun());
      case TrainrunSectionText.SourceArrival:
        return timeTag + TrainrunSectionsView.getTrainrunSectionTimeElementOddOffsetTag(
          trainrunSection.getSourceArrivalConsecutiveTime(), trainrunSection.getTrainrun());
      case TrainrunSectionText.TargetDeparture:
        return timeTag + TrainrunSectionsView.getTrainrunSectionTimeElementOddOffsetTag(
          trainrunSection.getTargetDepartureConsecutiveTime(), trainrunSection.getTrainrun());
      case TrainrunSectionText.TargetArrival:
        return timeTag + TrainrunSectionsView.getTrainrunSectionTimeElementOddOffsetTag(
          trainrunSection.getTargetArrivalConsecutiveTime(), trainrunSection.getTrainrun());
      case TrainrunSectionText.TrainrunSectionTravelTime:
        return timeTag;
      case TrainrunSectionText.TrainrunSectionName:
        return StaticDomTags.EDGE_LINE_TEXT_CLASS +
          TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(trainrunSection, selectedTrainrun, connectedTrainIds) +
  ...
  static extractTrainrunName(trainrunSection: TrainrunSection): string {
    return trainrunSection.getTrainrun().getCategoryShortName() + trainrunSection.getTrainrun().getTitle();
  }
  ...
  static extractTravelTime(trainrunSection: TrainrunSection, editorView: EditorView): string {
    ...
    if (trainrunSection.getTrainrun().selected() === true ||
      ...
      if (cumulativeTravelTime === trainrunSection.getTravelTime()) {
        // default case
        return TrainrunSectionsView.formatTime(trainrunSection.getTravelTime(), editorView.getTimeDisplayPrecision()) + '\'';
      } else {
        ...
        if (!editorView.isTemporaryDisableFilteringOfItemsInViewEnabled()) {
          ...
          if (TrainrunSectionsView.isBothSideNonStop(trainrunSection)) {
            ...
            if (!srcNonStopNode && !srcJunction) {
              const info = TrainrunSectionsView.calcVirtualSectionTimeForHiddenJunctions(cumTravelTimeData,
                trgNode, srcNode, editorView);
              if (info.isBreak) {
                if (TrainrunSectionsView.filterVirtualTravelTime(trgNode, info).virtualTravelTime === undefined) {
                  return '';
                }
                return '' + TrainrunSectionsView.formatTime(cumulativeTravelTime, editorView.getTimeDisplayPrecision()) +
                  '\' (' + TrainrunSectionsView.formatTime(info.virtualTravelTime, 1) + '\')';
              }
            }
            if (!trgNonStopNode && !trgJunction) {
              const info = TrainrunSectionsView.calcVirtualSectionTimeForHiddenJunctions(cumTravelTimeData,
                srcNode, trgNode, editorView);
              if (info.isBreak) {
                if (TrainrunSectionsView.filterVirtualTravelTime(srcNode, info).virtualTravelTime === undefined) {
                  return '';
                }
                return '' + TrainrunSectionsView.formatTime(cumulativeTravelTime, editorView.getTimeDisplayPrecision()) +
                  '\' (' + TrainrunSectionsView.formatTime(info.virtualTravelTime, 1) + '\')';
            ...
            if (srcNode.isNonStop(trainrunSection)) {
              if (!srcNonStopNode && !srcJunction) {
                const info = TrainrunSectionsView.calcVirtualSectionTimeForHiddenJunctions(cumTravelTimeData, trgNode, srcNode, editorView);
            ...
            if (trgNode.isNonStop(trainrunSection)) {
              if (!trgNonStopNode && !trgJunction) {
                const info = TrainrunSectionsView.calcVirtualSectionTimeForHiddenJunctions(cumTravelTimeData, srcNode, trgNode, editorView);
                if (info.virtualTravelTime === undefined) {
                  return '';
                }
                if (cumulativeTravelTime === info.virtualTravelTime) {
  ...
  static calcInternalVirtualSectionTimeForHiddenJunctions(cumulativeTravelTimeData,
                                                          startNode: Node,
                                                          nextNode: Node,
    ...
    while (idx < cumulativeTravelTimeData.length - 1) {
      ...
      if (editorView.checkFilterNode(breakAtNode)) {
        if (editorView.checkFilterNonStopNode(breakAtNode) || editorView.isJunctionNode(breakAtNode)) {
          break;
        }
      }
    }
  ...
  static calcVirtualSectionTimeForHiddenJunctions(cumulativeTravelTimeData: [],
                                                  startNode: Node, nextNode: Node,
                                                  editorView: EditorView) {
    let info =
      TrainrunSectionsView.calcInternalVirtualSectionTimeForHiddenJunctions(cumulativeTravelTimeData,
        startNode, nextNode, editorView);
    let virtualTravelTime: number = info.virtualTravelTime;
    if (virtualTravelTime === 0) {
      info =
        TrainrunSectionsView.calcInternalVirtualSectionTimeForHiddenJunctions(cumulativeTravelTimeData.reverse(),
          startNode, nextNode, editorView);
  ...
  static getTrainrunSectionValueToShow(trainrunSection: TrainrunSection, textElement: TrainrunSectionText, editorView: EditorView) {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
      case TrainrunSectionText.SourceArrival:
      case TrainrunSectionText.TargetDeparture:
  ...
  static getTrainrunSectionValueTextWidth(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): number {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
      case TrainrunSectionText.SourceArrival:
      case TrainrunSectionText.TargetDeparture:
      case TrainrunSectionText.TargetArrival:
      case TrainrunSectionText.TrainrunSectionTravelTime: {
        const data = TrainrunSectionsView.getFormattedDisplayTextWidth(trainrunSection, textElement);
        if (data !== undefined) {
          return data;
        }
  ...
  static getTrainrunSectionValueHtmlStyle(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): string {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
  ...
  static getTrainrunSectionNextAndDestinationNodeToShow(trainrunSection: TrainrunSection,
                                                        editorView: EditorView, atSource: boolean): string {
    let startNode: Node;
    if (atSource) {
  ...
  private static getFormattedDisplayTextColorRef(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): string {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
  ...
  getHiddenTagForTime(trainrunSection: TrainrunSection, textElement: TrainrunSectionText): boolean {
    ...
    switch (textElement) {
      ...
      case TrainrunSectionText.SourceArrival:
        ...
        if (!this.editorView.checkFilterNonStopNode(TrainrunSectionsView.getNode(trainrunSection, true))) {
          return true;
        }
      ...
      case TrainrunSectionText.TargetArrival:
        ...
        if (!this.editorView.checkFilterNonStopNode(TrainrunSectionsView.getNode(trainrunSection, false))) {
          return true;
        }
        if (this.editorView.isFilterShowNonStopTimeEnabled()) {
          return false;
      ...
      case TrainrunSectionText.TrainrunSectionTravelTime:
        ...
        return !trainrunSection.getTrainrun().selected() && TrainrunSectionsView.isBothSideNonStop(trainrunSection);
      case TrainrunSectionText.TrainrunSectionName: {
        ...
        const srcNode = TrainrunSectionsView.getNode(trainrunSection, true);
        const trgNode = TrainrunSectionsView.getNode(trainrunSection, false);
        if ((!this.editorView.checkFilterNonStopNode(srcNode)) || (!this.editorView.checkFilterNonStopNode(trgNode))) {
  ...
  setGroup(trainrunSectionGroup: d3.Selector) {
    ...
    this.trainrunSectionGroup = trainrunSectionGroup.append(StaticDomTags.GROUP_SVG).attr('class', 'TrainrunSectionsEdgeGroup');
  }

  createTrainrunSectionTextBackgrounds(groupEnter: d3.Selector,
                                       lineTextElement: TrainrunSectionText,
                                       selectedTrainrun: Trainrun,
                                       connectedTrainIds: any) {
    const atSource = lineTextElement === TrainrunSectionText.SourceArrival || lineTextElement === TrainrunSectionText.SourceDeparture;
    const isArrival = lineTextElement === TrainrunSectionText.SourceArrival || lineTextElement === TrainrunSectionText.TargetArrival;
    groupEnter
      .filter((d: TrainrunSectionViewObject) => this.filterTrainrunsectionAtNode(d.trainrunSection, atSource) &&
      ...
      .attr('class', (d: TrainrunSectionViewObject) => StaticDomTags.EDGE_LINE_TEXT_BACKGROUND_CLASS +
        TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(d.trainrunSection, selectedTrainrun, connectedTrainIds))
      .attr(StaticDomTags.EDGE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().getId())
      ...
      .attr('x', (d: TrainrunSectionViewObject) =>
        ...
        TrainrunSectionsView.getTrainrunSectionValueTextWidth(d.trainrunSection, lineTextElement) / 2)
      .attr('y', (d: TrainrunSectionViewObject) =>
        d.trainrunSection.getTextPositionY(lineTextElement) - TRAINRUN_SECTION_TEXT_AREA_HEIGHT / 2)
      .attr('width', (d: TrainrunSectionViewObject) => {
        return TrainrunSectionsView.getTrainrunSectionValueTextWidth(d.trainrunSection, lineTextElement);
      })
      .attr('height', TRAINRUN_SECTION_TEXT_AREA_HEIGHT)
      .classed(TrainrunSectionText[lineTextElement], true)
      .classed(StaticDomTags.TAG_HIDDEN, (d: TrainrunSectionViewObject) => this.getHiddenTagForTime(d.trainrunSection, lineTextElement));
  }

  createTrainrunSection(groupEnter: d3.Selector, classRef, selectedTrainrun: Trainrun, connectedTrainIds: any, enableEvents = true) {
    const trainrunSectionElements = groupEnter.append(StaticDomTags.EDGE_LINE_SVG)
      .attr('class', (d: TrainrunSectionViewObject) => StaticDomTags.EDGE_LINE_CLASS +
        TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(d.trainrunSection, selectedTrainrun, connectedTrainIds))
      .attr(StaticDomTags.EDGE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().getId())
      .attr('d', (d: TrainrunSectionViewObject) => D3Utils.getPathAsSVGString(this.transformPath(d.trainrunSection)))
      .classed(StaticDomTags.TAG_SELECTED, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().selected())
  ...
  createTrainrunsectionSemicircleAtNode(groupEnter: d3.Selector, selectedTrainrun: Trainrun, connectedTrainIds: any, atSource: boolean) {
    groupEnter
      .filter((d: TrainrunSectionViewObject) => this.filterTrainrunsectionAtNode(d.trainrunSection, atSource))
      .filter((d: TrainrunSectionViewObject) => {
        const trans = TrainrunSectionsView.getNode(d.trainrunSection, atSource).getTransition(d.trainrunSection.getId());
        if (trans === undefined) {
          return true;
        }
      ...
      .attr('class', (d: TrainrunSectionViewObject) => StaticDomTags.EDGE_LINE_STOP_ICON_CLASS + ' ' + StaticDomTags.TAG_FILL +
        TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(d.trainrunSection, selectedTrainrun, connectedTrainIds))
      .attr(StaticDomTags.EDGE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrunId())
      .attr('d', (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.createSemicircle(d.trainrunSection, TrainrunSectionsView.getPosition(d.trainrunSection, atSource)))
      .attr('transform', (d: TrainrunSectionViewObject) => 'translate(' +
      ...
      .attr(StaticDomTags.EDGE_NODE_ID, (d: TrainrunSectionViewObject) => TrainrunSectionsView.getNode(d.trainrunSection, atSource).getId())
  ...
  createTrainrunsectionSemicircles(groupEnter: d3.Selector, selectedTrainrun: Trainrun, connectedTrainIds: any) {
    if (this.editorView.isTemporaryDisableFilteringOfItemsInViewEnabled()) {
      return;
    }
    if (this.editorView.isFilterArrivalDepartureTimeEnabled()) {
    ...
    this.createTrainrunsectionSemicircleAtNode(groupEnter, selectedTrainrun, connectedTrainIds, false);
    this.createTrainrunsectionSemicircleAtNode(groupEnter, selectedTrainrun, connectedTrainIds, true);
  }

  createPinOnTrainrunsection(groupEnter: d3.Selector, selectedTrainrun: Trainrun, connectedTrainIds: any, atSource: boolean) {
    groupEnter.append(StaticDomTags.EDGE_LINE_PIN_SVG)
      .attr('class', StaticDomTags.EDGE_LINE_PIN_CLASS)
      .attr(StaticDomTags.EDGE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrunId())
      .attr('cx', (d: TrainrunSectionViewObject) => TrainrunSectionsView.getPosition(d.trainrunSection, atSource).getX())
      .attr('cy', (d: TrainrunSectionViewObject) => TrainrunSectionsView.getPosition(d.trainrunSection, atSource).getY())
      .attr('r', DEFAULT_PIN_RADIUS)
      .attr(StaticDomTags.EDGE_NODE_ID, (d: TrainrunSectionViewObject) => TrainrunSectionsView.getNode(d.trainrunSection, atSource).getId())
      .classed(StaticDomTags.TAG_HIDDEN, (d: TrainrunSectionViewObject) =>
        (!this.editorView.isTemporaryDisableFilteringOfItemsInViewEnabled() &&
          !this.editorView.checkFilterNonStopNode(TrainrunSectionsView.getNode(d.trainrunSection, atSource)))
      )
      ...
      .classed(StaticDomTags.TAG_SELECTED, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().selected())
      .on('mouseover', (d: TrainrunSectionViewObject, i, a) =>
        this.onTrainrunSectionMouseoverPin(TrainrunSectionsView.getNode(d.trainrunSection, atSource), a[i]))
      .on('mouseout', (d: TrainrunSectionViewObject, i, a) => this.onTrainrunSectionMouseoutPin(d.trainrunSection, a[i], atSource))
      .on('mousedown', () => this.onTrainrunSectionMousedownPin())
      .on('mousemove', () => this.onTrainrunSectionMousemovePin())
      .on('mouseup', (d: TrainrunSectionViewObject) => this.onTrainrunSectionMouseupPin(d.trainrunSection, atSource));
  }

  createTrainrunSectionElement(groupEnter: d3.Selector, selectedTrainrun: Trainrun,
                               connectedTrainIds: any, textElement: TrainrunSectionText,
                               enableEvents = true) {

    const atSource = textElement === TrainrunSectionText.SourceArrival || textElement === TrainrunSectionText.SourceDeparture;
    const isArrival = textElement === TrainrunSectionText.SourceArrival || textElement === TrainrunSectionText.TargetArrival;
    groupEnter
      .filter((d: TrainrunSectionViewObject) => this.filterTrainrunsectionAtNode(d.trainrunSection, atSource) &&
        this.filterTimeTrainrunsectionNonStop(d.trainrunSection, atSource, isArrival))
      .append(StaticDomTags.EDGE_LINE_TEXT_SVG)
      .attr('class', (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.getTrainrunSectionTimeElementClass(d.trainrunSection, textElement, selectedTrainrun, connectedTrainIds))
      .attr(StaticDomTags.EDGE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrunId())
      .attr(StaticDomTags.EDGE_LINE_TEXT_INDEX, textElement)
      .attr('x', (d: TrainrunSectionViewObject) => TrainrunSectionsView.getPositionX(d.trainrunSection, textElement))
      .attr('y', (d: TrainrunSectionViewObject) => TrainrunSectionsView.getPositionY(d.trainrunSection, textElement))
      .attr(TrainrunSectionsView.getAdditionPositioningAttr(textElement), (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.getAdditionPositioningValue(d.trainrunSection, textElement))
      .classed(StaticDomTags.TAG_SELECTED, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().selected())
      .classed(StaticDomTags.TAG_MUTED, (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.isMuted(d.trainrunSection, selectedTrainrun, connectedTrainIds))
      .classed(StaticDomTags.TAG_WARNING, (d: TrainrunSectionViewObject) => TrainrunSectionsView.hasWarning(d.trainrunSection, textElement))
      .classed(StaticDomTags.TAG_HIDDEN, (d: TrainrunSectionViewObject) => this.getHiddenTagForTime(d.trainrunSection, textElement))
      .classed(StaticDomTags.TAG_EVENT_DISABLED, !enableEvents)
      .text((d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.getTrainrunSectionValueToShow(d.trainrunSection, textElement, this.editorView))
  ...
  createTrainrunSectionGotoInfoElement(groupEnter: d3.Selector, selectedTrainrun: Trainrun,
                                       connectedTrainIds: any, atSource: boolean) {
    ...
    groupEnter
      ...
      .attr('transform', (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.getAdditionTextCloseToNodePositioningValue(d.trainrunSection, atSource))
      .classed(StaticDomTags.TAG_SELECTED, (d: TrainrunSectionViewObject) => d.trainrunSection.getTrainrun().selected())
      .classed(StaticDomTags.TAG_MUTED, (d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.isMuted(d.trainrunSection, selectedTrainrun, connectedTrainIds))
      .classed(StaticDomTags.TAG_WARNING, (d: TrainrunSectionViewObject) => TrainrunSectionsView.hasWarning(d.trainrunSection, textElement))
      ...
      .text((d: TrainrunSectionViewObject) =>
        TrainrunSectionsView.getTrainrunSectionNextAndDestinationNodeToShow(d.trainrunSection, this.editorView, atSource));
  }

  createNumberOfStopsTextElement(groupEnter: d3.Selector, trainrunSection: TrainrunSection,
                                 selectedTrainrun: Trainrun, connectedTrainIds: any, numberOfStops: number) {
    groupEnter.append(StaticDomTags.EDGE_LINE_TEXT_SVG)
      .attr('class', StaticDomTags.EDGE_LINE_TEXT_CLASS +
        ' ' + TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(trainrunSection, selectedTrainrun, connectedTrainIds) +
      ...
      .attr('x', 0.0)
      .attr('y', 0.0)
      .attr('transform', () => TrainrunSectionsView.translateAndRotateText(trainrunSection,
        TrainrunSectionText.TrainrunSectionNumberOfStops))
      .text(numberOfStops)
      .classed(StaticDomTags.TAG_MUTED, () => TrainrunSectionsView.isMuted(trainrunSection, selectedTrainrun, connectedTrainIds))
  ...
  createIntermediateStops(groupEnter: d3.Selector,
    ...
    const maxNumberOfStops = Math.min(SHOW_MAX_SINGLE_TRAINRUN_SECTIONS_STOPS, Vec2D.norm(lineOrientationVector) / 20);
    ...
    if (Vec2D.norm(lineOrientationVector) > maxWidth) {
      ...
      startPosition = Vec2D.add(startPosition, Vec2D.scale(Vec2D.normalize(lineOrientationVector), step));
    }

    if (numberOfStops <= maxNumberOfStops) {
      for (let stopIndex = 0; stopIndex < numberOfStops; stopIndex++) {
        this.createSingleStopElement(startPosition, lineOrientationVector, stopIndex, numberOfStops,
          groupEnter, selectedTrainrun, connectedTrainIds, numberOfStops, false);
      }
    } else {
      // collapsed view
      this.createSingleStopElement(startPosition, lineOrientationVector, 0, 1,
        groupEnter, selectedTrainrun, connectedTrainIds, numberOfStops, true);
      this.createNumberOfStopsTextElement(groupEnter, trainrunSection, selectedTrainrun, connectedTrainIds, numberOfStops);
    }
  }

  createAllIntermediateStops(groupEnter: d3.Selector, selectedTrainrun: Trainrun, connectedTrainIds: any) {
    groupEnter.each((t: TrainrunSectionViewObject, i, a) => {
      const grp = d3.select(a[i]).append(StaticDomTags.EDGE_LINE_STOPS_GROUP_SVG)
  ...
  createViewTrainrunSectionDataObjects(editorView: EditorView,
                                       inputTrainrunSections: TrainrunSection[],
                                       selectedTrainrun: Trainrun,
                                       connectedTrainIds: any): TrainrunSectionViewObject[] {
    const viewTrainrunSectionDataObjects: TrainrunSectionViewObject[] = [];
    inputTrainrunSections.forEach(
      (d: TrainrunSection) => {
  ...
  displayTrainrunSection(trainrunSections: TrainrunSection[]) {
    ...
    const group = this.trainrunSectionGroup
      ...
      .data(this.createViewTrainrunSectionDataObjects(this.editorView,
          filteredTrainrunSections, selectedTrainrun, connectedTrainIds),
        (d: TrainrunSectionViewObject) => d.key);

    const edgeRootContainerEnter = group.enter()
    ...
    this.defaultTrainrunSectionsRendering(groupLines, selectedTrainrun, connectedTrainIds, groupLabels);

    // Special case : render all trainrunsection which have one node filtered (hidden)
    this.oneNodeHiddenTrainrunSectionsRendering(groupLines, selectedTrainrun, connectedTrainIds, groupLabels);
  ...
  onIntermediateStopMouseOut(trainrunSection: TrainrunSection, stopIndex: number, position: Vec2D, domObj: any) {
  ...
  onIntermediateStopMouseOver(trainrunSection: TrainrunSection, stopIndex: number, position: Vec2D, domObj: any) {
    d3.event.stopPropagation();
    d3.select(domObj).classed(StaticDomTags.TAG_HOVER, true);
  }

  onIntermediateStopMouseDown(trainrunSection: TrainrunSection, stopIndex: number, position: Vec2D, domObj: any) {
  ...
  onTrainrunSectionElementClicked(trainrunSection: TrainrunSection, domObj: any, textElement: TrainrunSectionText) {
    d3.event.stopPropagation();
    ...
    this.onTrainrunSectionElementClickedNetzgrafikEditing(trainrunSection, textElement, clickPosition);
  }
  ...
  onTrainrunSectionMouseoutPin(trainrunSection: TrainrunSection, domObj: any, atSource: boolean) {
    ...
    const obj = d3.selectAll(StaticDomTags.EDGE_LINE_PIN_DOM_REF + '.' +
      (atSource ? StaticDomTags.EDGE_IS_TARGET : StaticDomTags.EDGE_IS_SOURCE))
      .filter((d: TrainrunSectionViewObject) => d.trainrunSection.getId() === trainrunSection.getId());
    ...
    this.editorView.trainrunSectionPreviewLineView.startPreviewLineAtPosition(
      TrainrunSectionsView.getNode(trainrunSection, !atSource), startAT);
    this.editorView.trainrunSectionPreviewLineView.updatePreviewLine();

    d3.selectAll(StaticDomTags.CONNECTION_LINE_PIN_DOM_REF).classed(StaticDomTags.CONNECTION_TAG_ONGOING_GDRAGGING, true);
  }
  ...
  onTrainrunSectionMouseupPin(trainrunSection: TrainrunSection, atSource: boolean) {
    d3.selectAll(StaticDomTags.CONNECTION_LINE_PIN_DOM_REF).classed(StaticDomTags.CONNECTION_TAG_ONGOING_GDRAGGING, false);
    this.createNewTrainrunSectionAfterPinDropped(TrainrunSectionsView.getNode(trainrunSection, atSource), trainrunSection);
    this.editorView.trainrunSectionPreviewLineView.stopPreviewLine();
  }

  private onTrainrunSectionElementClickedAnalytics(trainrunSection: TrainrunSection, textElement: TrainrunSectionText,
                                                   clickPos: Vec2D) {
    let node: Node;
    let minute: number;
    switch (textElement) {
    ...
    this.editorView.calculateShortestDistanceNodesFromStartingTrainrunSection(trainrunSection.getId(), node.getId());
  }

  private onTrainrunSectionElementClickedNetzgrafikEditing(trainrunSection: TrainrunSection, textElement: TrainrunSectionText,
                                                           clickPos: Vec2D) {
    switch (textElement) {
      case TrainrunSectionText.SourceDeparture:
      case TrainrunSectionText.SourceArrival:
  ...
  private filterTimeTrainrunsectionNonStop(trainrunSection: TrainrunSection, atSource: boolean, isArrival: boolean): boolean {
    if (!isArrival) {
      return true;
    }
    if (atSource) {
  ...
  private filterTrainrunsectionAtNode(trainrunSection: TrainrunSection, atSource: boolean): boolean {
    if (this.editorView.isTemporaryDisableFilteringOfItemsInViewEnabled()) {
      return true;
    }
  ...
  private transformPathAddExtraElementForPortAlignmentBottom(node: Node, ts: TrainrunSection, element, transformedPath: Vec2D[]): Vec2D[] {
  ...
  private transformPathIfSourceNodeFilteredDueNonStopNodesFiltering(ts: TrainrunSection, transformedPath: Vec2D[]): Vec2D[] {
    ...
    if (!this.editorView.checkFilterNonStopNode(srcNode)) {
      ...
      transformedPath = this.transformPathAddExtraElementForPortAlignmentBottom(srcNode, ts, element, transformedPath);
      transformedPath = transformedPath.reverse();
  ...
  private transformPathIfTargetNodeFilteredDueNonStopNodesFiltering(ts: TrainrunSection, transformedPath: Vec2D[]): Vec2D[] {
    const trgNode = ts.getTargetNode();
    const path: Vec2D[] = Object.assign([], ts.getPath());

    if (!this.editorView.checkFilterNonStopNode(trgNode)) {
      const element = path[3].copy();
      transformedPath = this.transformPathAddExtraElementForPortAlignmentBottom(trgNode, ts, element, transformedPath);
  ...
  private filterOutAllTrainrunSectionWithHiddenNodeConnection(trainrunSection: TrainrunSection): boolean {
    if (this.editorView.isTemporaryDisableFilteringOfItemsInViewEnabled()) {
      return true;
    }
  ...
  private oneNodeHiddenTrainrunSectionsRendering(inGroupLines, selectedTrainrun: Trainrun, connectedTrainIds: any[], inGroupLabels) {
    const groupLines =
      inGroupLines.filter((d: TrainrunSectionViewObject) => !this.filterOutAllTrainrunSectionWithHiddenNodeConnection(d.trainrunSection));

    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_0, selectedTrainrun, connectedTrainIds, false);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_1, selectedTrainrun, connectedTrainIds, false);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_2, selectedTrainrun, connectedTrainIds, false);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_3, selectedTrainrun, connectedTrainIds, false);

    if (!this.editorView.isElementDragging()) {
      const groupLabels =
        inGroupLabels.filter((d: TrainrunSectionViewObject) => !this.filterOutAllTrainrunSectionWithHiddenNodeConnection(d.trainrunSection));

      this.createTrainrunSectionTextBackgrounds(groupLabels, TrainrunSectionText.SourceArrival, selectedTrainrun, connectedTrainIds);
      this.createTrainrunSectionTextBackgrounds(groupLabels, TrainrunSectionText.SourceDeparture, selectedTrainrun, connectedTrainIds);
      this.createTrainrunSectionTextBackgrounds(groupLabels, TrainrunSectionText.TargetArrival, selectedTrainrun, connectedTrainIds);
      this.createTrainrunSectionTextBackgrounds(groupLabels, TrainrunSectionText.TargetDeparture, selectedTrainrun, connectedTrainIds);

      this.createTrainrunSectionElement(groupLabels, selectedTrainrun,
        connectedTrainIds, TrainrunSectionText.SourceArrival, false);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun,
        connectedTrainIds, TrainrunSectionText.SourceDeparture, false);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun,
        connectedTrainIds, TrainrunSectionText.TargetArrival, false);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun,
        connectedTrainIds, TrainrunSectionText.TargetDeparture, false);

      this.createTrainrunSectionGotoInfoElement(groupLabels, selectedTrainrun, connectedTrainIds, true);
      this.createTrainrunSectionGotoInfoElement(groupLabels, selectedTrainrun, connectedTrainIds, false);

      this.createTrainrunsectionSemicircles(groupLabels, selectedTrainrun, connectedTrainIds);
    }

  }

  private defaultTrainrunSectionsRendering(inGroupLines, selectedTrainrun: Trainrun, connectedTrainIds: any[], inGroupLabels) {
    const groupLines =
      inGroupLines.filter((d: TrainrunSectionViewObject) => this.filterOutAllTrainrunSectionWithHiddenNodeConnection(d.trainrunSection));

    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_0, selectedTrainrun, connectedTrainIds);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_1, selectedTrainrun, connectedTrainIds);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_2, selectedTrainrun, connectedTrainIds);
    this.createTrainrunSection(groupLines, StaticDomTags.EDGE_LINE_LAYER_3, selectedTrainrun, connectedTrainIds);

    if (!this.editorView.isElementDragging()) {
      const groupLabels =
        inGroupLabels.filter((d: TrainrunSectionViewObject) => this.filterOutAllTrainrunSectionWithHiddenNodeConnection(d.trainrunSection));

      this.createPinOnTrainrunsection(groupLabels, selectedTrainrun, connectedTrainIds, true);
      this.createPinOnTrainrunsection(groupLabels, selectedTrainrun, connectedTrainIds, false);
      ...
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.SourceArrival);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.SourceDeparture);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.TargetArrival);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.TargetDeparture);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.TrainrunSectionTravelTime);
      this.createTrainrunSectionElement(groupLabels, selectedTrainrun, connectedTrainIds, TrainrunSectionText.TrainrunSectionName);

      this.createTrainrunsectionSemicircles(groupLabels, selectedTrainrun, connectedTrainIds);

      this.createAllIntermediateStops(groupLabels, selectedTrainrun, connectedTrainIds);
    }
  }

  private createSingleStopElement(startPosition: Vec2D, lineOrientationVector: Vec2D,
                                  stopIndex: number, drawNumberOfStops,
                                  groupEnter: d3.Selector,
                                  selectedTrainrun: Trainrun,
                                  connectedTrainIds: any,
                                  numberOfStops: number,
                                  collapsedStops: boolean) {
    const position = Vec2D.add(startPosition, Vec2D.scale(lineOrientationVector, (stopIndex + 1.0) / (drawNumberOfStops + 1.0)));
    groupEnter.append(StaticDomTags.EDGE_LINE_STOPS_SVG)
      .attr('class', (t: TrainrunSectionViewObject) => StaticDomTags.EDGE_LINE_STOPS_CLASS +
        TrainrunSectionsView.createTrainrunSectionFrequencyClassAttribute(t.trainrunSection, selectedTrainrun, connectedTrainIds))
      .attr(StaticDomTags.EDGE_ID, (t: TrainrunSectionViewObject) => t.trainrunSection.getId())
      .attr(StaticDomTags.EDGE_LINE_LINE_ID, (t: TrainrunSectionViewObject) => t.trainrunSection.getTrainrun().getId())
      ...
      .classed(StaticDomTags.TAG_SELECTED, (t: TrainrunSectionViewObject) => t.trainrunSection.getTrainrun().selected())
  ...
  private createNewTrainrunSectionAfterPinDropped(endNode: any, trainrunSection: TrainrunSection) {
    ...
    d3.event.stopPropagation();
    const trainrunSectionFrom = this.editorView.trainrunSectionPreviewLineView.getExistingTrainrunSection();
