...
export class D3Utils {
  ...
  static bringTrainrunSectionToFront() {
    ...
    d3.selectAll(StaticDomTags.EDGE_ROOT_CONTAINER_DOM_REF + '.' + StaticDomTags.TAG_MUTED).lower();
    d3.selectAll(StaticDomTags.EDGE_ROOT_CONTAINER_DOM_REF + '.' + StaticDomTags.TAG_HOVER).raise();
    d3.selectAll(StaticDomTags.EDGE_ROOT_CONTAINER_DOM_REF + '.' + StaticDomTags.TAG_SELECTED).raise();
  }
  ...
  static updateTrainrunSectionPreviewLine(startPos: Vec2D) {
    ...
    const fromPos: Vec2D = Vec2D.add(startPos, Vec2D.scale(Vec2D.normalize(Vec2D.sub(mousePosVec, startPos)), 8.0));
    d3.selectAll(StaticDomTags.PREVIEW_LINE_DOM_REF)
      .attr('d', 'M' + fromPos.getX() + ',' + fromPos.getY() + 'L' + mousePosVec.getX() + ',' + mousePosVec.getY());
  }
  ...
  static updateIntermediateStopOrTransitionPreviewLine(startPos: Vec2D, endPos: Vec2D) {
    ...
    const mousePosVec: Vec2D = new Vec2D(mousePos[0], mousePos[1]);
    const fromPos: Vec2D = Vec2D.add(startPos, Vec2D.scale(Vec2D.normalize(Vec2D.sub(mousePosVec, startPos)), 8.0));
    const toPos: Vec2D = Vec2D.add(endPos, Vec2D.scale(Vec2D.normalize(Vec2D.sub(mousePosVec, endPos)), 8.0));
    d3.selectAll(StaticDomTags.PREVIEW_LINE_DOM_REF)
      .attr('d', 'M' + fromPos.getX() + ',' + fromPos.getY() +
        'L' + mousePosVec.getX() + ',' + mousePosVec.getY() +
        'L' + toPos.getX() + ',' + toPos.getY()
      );
  }
  ...
  static updateConnectionPreviewLine(startPos: Vec2D) {
    ...
    const mousePosVec: Vec2D = new Vec2D(mousePos[0], mousePos[1]);
    const fromPos: Vec2D = Vec2D.add(startPos, Vec2D.scale(Vec2D.normalize(Vec2D.sub(mousePosVec, startPos)), 8.0));
    const toPos: Vec2D = Vec2D.add(mousePosVec, Vec2D.scale(Vec2D.normalize(Vec2D.sub(startPos, mousePosVec)), 8.0));
    d3.selectAll(StaticDomTags.PREVIEW_CONNECTION_LINE_DOM_REF)
      .attr('d', 'M' + fromPos.getX() + ',' + fromPos.getY() + 'L' + toPos.getX() + ',' + toPos.getY());
  }

  static disableTrainrunSectionForEventHandling() {
    d3.selectAll(StaticDomTags.EDGE_LINE_TEXT_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, true);
    d3.selectAll(StaticDomTags.EDGE_LINE_TEXT_BACKGROUND_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, true);
    d3.selectAll(StaticDomTags.EDGE_LINE_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, true);
    d3.selectAll(StaticDomTags.EDGE_LINE_PIN_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, true);
  }

  static resetTrainrunSectionForEventHandling() {
    d3.selectAll(StaticDomTags.EDGE_LINE_TEXT_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, false);
    d3.selectAll(StaticDomTags.EDGE_LINE_TEXT_BACKGROUND_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, false);
    d3.selectAll(StaticDomTags.EDGE_LINE_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, false);
    d3.selectAll(StaticDomTags.EDGE_LINE_PIN_DOM_REF).classed(StaticDomTags.EDGE_DISABLE_EVENTS, false);
  }

  static highlightNode(node: Node) {
    d3.selectAll(StaticDomTags.NODE_ROOT_DOM_REF)
      .filter((n: NodeViewObject) => n.node.getId() === node.getId()).classed(StaticDomTags.NODE_HIGHLIGHT, true);
  }

  static unhighlightNode(node: Node) {
    d3.selectAll(StaticDomTags.NODE_ROOT_DOM_REF)
      .filter((n: NodeViewObject) => n.node.getId() === node.getId()).classed(StaticDomTags.NODE_HIGHLIGHT, false);
  }
  ...
  static enableSpecialEditing(isTopologyOrNodeEditing: boolean) {
    ...
    rootElement.selectAll('path').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, true);
    rootElement.selectAll('rect').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, true);
    rootElement.selectAll('circle').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, true);
    rootElement.selectAll('line').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, true);
    rootElement.selectAll('text').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, true);

    d3.selectAll(StaticDomTags.NODE_ROOT_DOM_REF)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    d3.selectAll(StaticDomTags.NODE_BACKGROUND_DOM_REF)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    d3.selectAll(StaticDomTags.NODE_DOCKABLE_DOM_REF)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    d3.selectAll(StaticDomTags.NODE_LABELAREA_TEXT_DOM_REF)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);

    d3.selectAll(StaticDomTags.NOTE_ROOT_DOM_REF)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    d3.selectAll(StaticDomTags.NOTE_HOVER_ROOT_SVG)
      .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);

    if (isTopologyOrNodeEditing) {
      d3.selectAll(StaticDomTags.NODE_CONNECTIONTIME_TEXT_DOM_REF)
        .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
      d3.selectAll(StaticDomTags.NODE_EDIT_AREA_BACKGROUND_DOM_REF)
        .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
  ...
  static disableSpecialEditing() {
    ...
    const rootElement = d3.select('#graphContainer');
    rootElement.selectAll('path').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    rootElement.selectAll('rect').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    rootElement.selectAll('circle').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    rootElement.selectAll('line').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
    rootElement.selectAll('text').classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
  ...
  static resetShortestDistanceRenderer() {
    ...
    const rootElement = d3.select('#graphContainer');
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_0, false);
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_1, false);
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_2, false);
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_3, false);
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_4, false);
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF).classed(StaticDomTags.TAG_ANALYTICS_LEVEL_5, false);
    ...
    rootElement.selectAll('text').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_CLICKABLE, false);
    rootElement.selectAll('text').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_MUTED, false);
  ...
  static initShortestDistanceRenderer() {
    ...
    rootElement.selectAll('text.edge_text').classed(StaticDomTags.TAG_ANALYTICS_ENFORCE_DISABLE, true);
    rootElement.selectAll('text.edge_text').classed(StaticDomTags.TAG_EVENT_DISABLED, true);

    d3.selectAll('text.edge_text.SourceDeparture').classed(StaticDomTags.TAG_ANALYTICS_ENFORCE_DISABLE, false);
    d3.selectAll('text.edge_text.TargetDeparture').classed(StaticDomTags.TAG_ANALYTICS_ENFORCE_DISABLE, false);
    ...
    d3.selectAll('text.edge_text.SourceDeparture').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_CLICKABLE, true);
    d3.selectAll('text.edge_text.TargetDeparture').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_CLICKABLE, true);

    d3.selectAll('text.edge_text').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_FOCUSED, false);

    d3.selectAll(StaticDomTags.NODE_HOVER_ROOT_DOM_REF).classed(StaticDomTags.TAG_EVENT_DISABLED, false);

    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_TEXT_LEFT_DOM_REF)
      .text('')
      .raise();
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_TEXT_RIGHT_DOM_REF)
      .text('')
      .raise();
    rootElement.selectAll(StaticDomTags.NODE_ANALYTICSAREA_DOM_REF)
      .raise();
  }
  ...
  static shortestDistanceRenderer(finalShortestDistanceNodes: ShortestDistanceNode[]) {
    ...
    if (finalShortestDistanceNodes.length > 0) {
      d3.selectAll('text.edge_text.SourceDeparture').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_MUTED, true);
      d3.selectAll('text.edge_text.TargetDeparture').classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_MUTED, true);
    }
    ...
    finalShortestDistanceNodes.forEach((sdn: ShortestDistanceNode) => {
      ...
      sdn.path.reverse().forEach((ts: TrainrunSection) => {
        ...
        rootElement.selectAll('path.edge_line[id="' + ts.getId() + '"]')
          .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
        rootElement.selectAll('text.edge_text[id="' + ts.getId() + '"]')
        ...
        rootElement.selectAll('circle.edge_line_stops[id="' + ts.getId() + '"]')
          .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
        ...
        if (ts.getTargetNodeId() === endNodeId) {
          d3.selectAll('text.edge_text.TargetDeparture[id="' + ts.getId() + '"]')
            .classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_MUTED, true);
          d3.selectAll('text.edge_text.SourceArrival[id="' + ts.getId() + '"]')
            .classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_MUTED, true);
          d3.selectAll('text.edge_text.SourceDeparture[id="' + ts.getId() + '"]')
            .classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_FOCUSED, true);
          d3.selectAll('text.edge_text.TargetArrival[id="' + ts.getId() + '"]')
            .classed(StaticDomTags.TAG_ANALYTICS_TRAINRUNSECTION_TEXT_FOCUSED, true);
        ...
        if (srcTrans !== undefined) {
          ...
          if ((finalTrainrunSections.find((checkTs: TrainrunSection) => checkTs.getId() === srcId1) !== undefined) &&
            (finalTrainrunSections.find((checkTs: TrainrunSection) => checkTs.getId() === srcId2) !== undefined)) {
            rootElement.selectAll('path.transition_line[' +
              StaticDomTags.TRANSITION_TRAINRUNSECTION_ID1 + '="' + srcId1 + '"][' +
              StaticDomTags.TRANSITION_TRAINRUNSECTION_ID2 + '="' + srcId2 + '"]')
              .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
          }
        }
        if (trgTrans !== undefined) {
          ...
          if ((finalTrainrunSections.find((checkTs: TrainrunSection) => checkTs.getId() === trgId1) !== undefined) &&
            (finalTrainrunSections.find((checkTs: TrainrunSection) => checkTs.getId() === trgId2) !== undefined)) {
            rootElement.selectAll('path.transition_line[' +
              StaticDomTags.TRANSITION_TRAINRUNSECTION_ID1 + '="' + trgId1 + '"][' +
              StaticDomTags.TRANSITION_TRAINRUNSECTION_ID2 + '="' + trgId2 + '"]')
              .classed(StaticDomTags.TAG_ENFORCE_DISABLE_ALL_POINTER_EVENTS, false);
          }
        }
      });
    ...
    rootElement.selectAll('rect.node_analyticsarea').classed(StaticDomTags.TAG_ANALYTICS_LEVEL_5, true);
    ...
    finalShortestDistanceNodes.forEach((sdn: ShortestDistanceNode) => {
      const level = Math.max(0, sdn.path
        .map((ts: TrainrunSection) => ts.getTrainrunId())
        .filter((v, i, a) => a.indexOf(v) === i)
        .length - 1);

      rootElement.selectAll('rect.node_analyticsarea[id="' + sdn.node.getId() + '"]')
        .classed(StaticDomTags.TAG_ANALYTICS_LEVEL_5, false)
        .classed(StaticDomTags.TAG_ANALYTICS_LEVEL + Math.min(5, level), true);

      rootElement.selectAll('text.node_title_analyticsarea_left[id="' + sdn.node.getId() + '"]')
        .text('#' + level);

      rootElement.selectAll('text.node_title_analyticsarea_right[id="' + sdn.node.getId() + '"]')
