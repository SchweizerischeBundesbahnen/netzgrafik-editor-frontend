...
export class EditorMainViewComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private nodeService: NodeService,
    ...
    this.editorView = new EditorView(this,
      nodeService, trainrunService, trainrunSectionService, noteService, filterService, uiInteractionService, undoSerivce, copyService, logService);
    this.uiInteractionService.zoomInObservable.pipe(takeUntil(this.destroyed)).subscribe(
      (zoomCenter: Vec2D) => this.editorView.svgMouseController.zoomIn(zoomCenter));
    this.uiInteractionService.zoomOutObservable.pipe(takeUntil(this.destroyed)).subscribe(
      (zoomCenter: Vec2D) => this.editorView.svgMouseController.zoomOut(zoomCenter));
    this.uiInteractionService.zoomResetObservable.pipe(takeUntil(this.destroyed)).subscribe(
      (zoomCenter: Vec2D) => this.editorView.svgMouseController.zoomReset(zoomCenter));
    this.uiInteractionService.setEditorModeObservable.pipe(takeUntil(this.destroyed)).subscribe(
      (mode: number) => this.editorView.setEditorMode(mode));
    this.uiInteractionService.moveNetzgrafikEditorViewFocalPointObservable.pipe(takeUntil(this.destroyed)).subscribe(
      (center: Vec2D) => this.moveNetzgrafikEditorViewFocalPoint(center));
  }
  ...
  bindViewToServices() {
    this.editorView.bindAddNode(
      (positionX: number, positionY: number) => this.nodeService.addNodeWithPosition(positionX, positionY));

    this.editorView.bindGetNodePathToEnd(
      (node: Node, trainrunSection: TrainrunSection) => this.trainrunService.getNodePathToEnd(node, trainrunSection));
    ...
    this.editorView.bindAddTrainrunSectionWithSourceTarget((
      ...
      this.trainrunSectionService.createTrainrunSection(sourceNode.getId(), targetNode.getId());
    ...
    this.editorView.bindDeleteTrainrunSection(
      (trainrunSection: TrainrunSection) => this.trainrunSectionService.deleteTrainrunSection(trainrunSection.getId()));
    ...
    this.editorView.bindUndockTransition(
      (nodeId: number, transitionId: number) => this.nodeService.undockTransition(nodeId, transitionId)
    );
    this.editorView.bindSetTrainrunAsSelected(
      (trainrun: Trainrun) => this.trainrunService.setTrainrunAsSelected(trainrun.getId()));

    this.editorView.bindClickSelectedTrainrunSection(
      (informSelectedTrainrunClick: InformSelectedTrainrunClick) => this.trainrunSectionService.clickSelectedTrainrunSection(informSelectedTrainrunClick));
    ...
    this.editorView.bindGetCumulativeTravelTime(
      (trainrunSection: TrainrunSection) => this.trainrunService.getCumulativeTravelTime(trainrunSection));

    this.editorView.bindGetCumulativeTravelTimeAndNodePath(
      (trainrunSection: TrainrunSection) => this.trainrunService.getCumulativeTravelTimeAndNodePath(trainrunSection));
    ...
    this.editorView.bindRemoveConnectionFromNode(
      (connection: Connection, node: Node) => this.nodeService.removeConnectionFromNode(node.getId(), connection.getId()));
    ...
    this.editorView.bindShowTrainrunInformation((trainrunSection: TrainrunSection, position: Vec2D) => {
      this.trainrunService.setTrainrunAsSelected(trainrunSection.getTrainrun().getId());
      this.trainrunSectionService.setTrainrunSectionAsSelected(trainrunSection.getId());
      const parameter = new TrainrunDialogParameter(TrainrunDialogType.TRAINRUN_DIALOG, position);
      this.uiInteractionService.showTrainrunDialog(parameter);
    ...
    this.editorView.bindShowTrainrunSectionInformation((trainrunSection: TrainrunSection, position: Vec2D,
    ...
    this.editorView.bindGetConnectedTrainrunIds(
      (trainrun: Trainrun) => this.trainrunService.getConnectedTrainrunIdsFirstOrder(trainrun.getId()));
    ...
    this.editorView.bindGetNodeFromTransition(
      (t: Transition) => this.nodeService.getNodeFromTransition(t));

    this.editorView.bindGetNodeFromConnection(
      (c: Connection) => this.nodeService.getNodeForConnection(c));
    ...
    this.editorView.bindIsfilterTrainrunCategoryEnabled(
      (trainrunCatgory: TrainrunCategory) => this.filterService.isFilterTrainrunCategoryEnabled(trainrunCatgory));

    this.editorView.bindCheckFilterNode(
      (node: Node) => (this.filterService.checkFilterNode(node) &&
        this.filterService.checkFilterEmptyNode(node)));

    this.editorView.bindFilterNode(
      (node: Node) => this.filterService.filterNode(node));

    this.editorView.bindFilterNote(
      (note: Note) => this.filterService.filterNote(note));

    this.editorView.bindCheckFilterNonStopNode(
      (node: Node) => this.filterService.checkFilterNonStopNode(node));

    this.editorView.bindIsNodeVisible(
      (node: Node) => this.filterService.isNodeVisible(node));

    this.editorView.bindIsJunctionNode(
      (node: Node) => this.filterService.isJunctionNode(node));

    this.editorView.bindFilterTrainrunsection(
      (trainrunSection: TrainrunSection) => this.filterService.filterTrainrunsection(trainrunSection));
    ...
    this.editorView.bindFilterTrainrun(
      (trainrun: Trainrun) => this.filterService.filterTrainrun(trainrun));
    ...
    this.editorView.bindIsFilterTrainrunFrequencyEnabled(
      (trainrunFrequency: TrainrunFrequency) => this.filterService.isFilterTrainrunFrequencyEnabled(trainrunFrequency));
    ...
    this.editorView.bindReplaceIntermediateStopWithNode(
      (trainsectionId: number, stopIndex: number, nodeId: number) =>
        this.trainrunSectionService.replaceIntermediateStopWithNode(trainsectionId, stopIndex, nodeId));

    this.editorView.bindGetTimeDisplayPrecision(
      () => this.filterService.getTimeDisplayPrecision()
    );

    this.editorView.bindSetTimeDisplayPrecision(
      (precision) => this.filterService.setTimeDisplayPrecision(precision)
    );

    this.editorView.bindSelectNode(
      (nodeId, enforceUpdate = true) => this.nodeService.selectNode(nodeId, enforceUpdate));

    this.editorView.bindUnselectNode(
      (nodeId, enforceUpdate = true) => this.nodeService.unselectNode(nodeId, enforceUpdate));

    this.editorView.bindUnselectAllNodes(
      (enforceUpdate = true) => this.nodeService.unselectAllNodes(enforceUpdate));

    this.editorView.bindIsNodeSelected(
      (nodeId) => this.nodeService.isNodeSelected(nodeId));

    this.editorView.bindSelectNote(
      (noteId) => this.noteService.selectNote(noteId));

    this.editorView.bindUnselectNote(
      (noteId) => this.noteService.unselectNote(noteId));

    this.editorView.bindUnselectAllNotes(
      () => this.noteService.unselectAllNotes());

    this.editorView.bindIsNoteSelected(
      (noteId) => this.noteService.isNoteSelected(noteId));

    this.editorView.bindAddNote(
      (position: Vec2D, clickPosition: Vec2D) => {
    ...
    this.editorView.bindMoveNote(
      (inputNoteId: number, newPosition: Vec2D, round: number, dragEnd: boolean) => {
        this.noteService.moveNote(inputNoteId, newPosition.getX(), newPosition.getY(), round, dragEnd);
      });

    this.editorView.bindGetNoteLayerIndex(
      (noteId: number) => this.noteService.getNoteLayerIndex(noteId));
    ...
    this.editorView.bindCalculateShortestDistanceNodesFromStartingTrainrunSection(
      (trainrunSectionId: number, departureNodeId: number) => {
        this.analyticsService.calculateShortestDistanceNodesFromStartingTrainrunSection(trainrunSectionId, departureNodeId);
      });
