...
export class VariantsViewComponent implements OnDestroy {
  ...
  readonly variants = combineLatest([this.projectSubject, this.searchQuery, this.showArchiveControl.valueChanges.pipe(startWith(false))])
    .pipe(map(([project, searchQuery, showArchive]) =>
      project.variants.filter((v: VariantSummaryDto) => {
        if (!searchQuery) {
          return v.isArchived === showArchive;
        }
        return v.isArchived === showArchive && this.getTitleCurrentVersion(v).toLocaleLowerCase().includes(searchQuery);
      }))
    ).pipe(
      map(variantSummaryDto => variantSummaryDto
        .sort(this.sortByPVariantSummaryDtoName)
      )
    );

  readonly projectDataActions: Observable<SlotAction[] | undefined> = this.projectSubject.pipe(map(project => {
  ...
  onArchiveVariantClicked(variantToEdit: VariantSummaryDto) {
    this.uiInteractionService.showConfirmationDiagramDialog(new ConfirmationDialogParameter(
      ...
      .subscribe(project => {
        this.versionControlService.archiveVariantWithId(variantToEdit.id).subscribe(
          () => {
            this.projectService.getProject(variantToEdit.projectId).subscribe(
              (projectTdo) => this.updateProject(projectTdo)
            );
          });
      });
  }
  ...
  onAddVariantClicked(): void {
    VariantDialogComponent.open(this.dialog, {name: ''})
      ...
      ).subscribe(variantId => this.navigationService.navigateToEditor(this.project.id, variantId));
  }
