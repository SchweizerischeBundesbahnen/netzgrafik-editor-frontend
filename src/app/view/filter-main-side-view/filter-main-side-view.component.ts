...
export class FilterMainSideViewComponent implements OnInit, OnDestroy {
  ...
  get showPerlenkette(): boolean {
    return this.mode === LayoutMode.PERLENKETTE_ONLY || this.mode === LayoutMode.FILTER_AND_PERLENKETTE;
  }

  ngOnInit(): void {
    this.uiInteractionService.nodeStammdatenWindow.pipe(takeUntil(this.destroyed)).subscribe((open: boolean) => {
      if (open) {
        if (this.mode === LayoutMode.MAIN_FULL || this.mode === LayoutMode.DETAIL_ONLY || this.mode === LayoutMode.PERLENKETTE_ONLY) {
    ...
    this.uiInteractionService.filterWindow.pipe(takeUntil(this.destroyed)).subscribe((type: FilterWindowType | null) => {
      this.type = type;
      if (type) {
    ...
    this.uiInteractionService.perlenketteWindow.pipe(takeUntil(this.destroyed)).subscribe((open: boolean) => {
      if (open) {
        if (this.mode === LayoutMode.MAIN_FULL || this.mode === LayoutMode.PERLENKETTE_ONLY || this.mode === LayoutMode.DETAIL_ONLY) {
          this.setMode(LayoutMode.PERLENKETTE_ONLY);
        } else {
          this.setMode(LayoutMode.FILTER_AND_PERLENKETTE);
        }
    ...
    this.uiInteractionService.streckengrafikWindow.pipe(takeUntil(this.destroyed)).subscribe((mainViewMode: MainViewMode) => {
