...
export class FilterableLabelDialogComponent implements OnDestroy {
  ...
  constructor(
    ...
    public readonly dialogRef: SbbDialogRef<FilterableLabelDialogComponent, FilterableLabelsFormComponentModel>,
    @Inject(SBB_DIALOG_DATA) data: FilterableLabelsFormComponentModel
  ) {

    data.updateLabelCallback = () => this.updateLabel();

    this.formmodel = new FormModel<FilterableLabelsFormComponentModel>(data ?? {
      name: '',
      dialogTitel: '',
    ...
    this.dialogRef.beforeClosed().pipe(takeUntil(this.destroyed)).subscribe(() => {
      this.updateLabel();
    });
  }

  public static open(dialog: SbbDialog, initData: FilterableLabelsFormComponentModel): Observable<FilterableLabelsFormComponentModel> {
    return dialog.open(FilterableLabelDialogComponent, {data: initData})
      .afterClosed();
  }
  ...
  onTransferClicked(): void {
    ...
    const dialogContent = 'Soll das Label "' +
      this.originalLabel + '" auf alle sichtbaren ' +
      this.dialogTitel + ' übertragen werden?';
  ...
  onDeleteClicked(): void {
    ...
    const dialogContent = 'Soll das Label "' +
      this.originalLabel + '" definitiv aus allen sichtbaren ' +
      this.dialogTitel + ' gelöscht werden?';
    const confirmationDialogParamter = new ConfirmationDialogParameter(dialogTitle, dialogContent);
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
