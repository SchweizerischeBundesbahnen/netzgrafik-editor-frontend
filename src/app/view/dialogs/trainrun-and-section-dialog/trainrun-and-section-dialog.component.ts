...
export class TrainrunAndSectionDialogComponent implements OnDestroy {
  ...
  @ViewChild('trainrunAndSectionEditorTabsViewTemplate', {static: true}) trainrunAndSectionEditorTabsViewTemplate: TemplateRef<any>;
  ...
  constructor(
    ...
    this.uiInteractionService.trainrunDialog.pipe(takeUntil(this.destroyed)).subscribe(parameter => {
      this.data = parameter;
      ...
      const leftNode = this.trainrunSectionHelper.getLeftNode(selectedTrainrunSection, parameter.nodesOrdered);
      const rightNode = this.trainrunSectionHelper.getRightNode(selectedTrainrunSection, parameter.nodesOrdered);
      this.selectedTrainrunSectionName = leftNode.getFullName() + ' — ' + rightNode.getFullName();

      this.openDialog(parameter);
    });
  }

  static getDialogConfig(parameter: TrainrunDialogParameter, trainrunFrequency: TrainrunFrequency[]): SbbDialogConfig {
    const width = 630;
    let height = 405;
  ...
  openDialog(parameter: TrainrunDialogParameter) {
    ...
    this.dialogConfig = TrainrunAndSectionDialogComponent.getDialogConfig(parameter, this.dataService.getTrainrunFrequencies());
    this.dialogRef = this.dialog.open(this.trainrunAndSectionEditorTabsViewTemplate, this.dialogConfig);
    this.dialogPos = {
