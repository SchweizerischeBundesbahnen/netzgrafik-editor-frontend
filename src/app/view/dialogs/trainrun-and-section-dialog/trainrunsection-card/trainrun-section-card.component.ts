...
export class TrainrunSectionCardComponent implements AfterViewInit, OnDestroy {
  ...
  private isTowardTarget(targetSide: "right" | "left"): boolean {
    if (targetSide === "right") {
      return (
        this.selectedTrainrunSection.getTargetNode() ===
        this.trainrunSectionHelper.getRightNode(this.selectedTrainrunSection)
      );
    } else {
      return (
        this.selectedTrainrunSection.getTargetNode() ===
        this.trainrunSectionHelper.getLeftNode(this.selectedTrainrunSection)
      );
    }
  }
}
