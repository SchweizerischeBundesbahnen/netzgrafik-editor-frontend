...
export class TrainrunTabComponent implements OnDestroy {
  ...
  getTimeCategoryClassname(trainrunTimeCategory: TrainrunTimeCategory): string {
    if (trainrunTimeCategory.id === this.selectedTimeCategory.id) {
      return 'TrainrunDialog TimeCategory ' +
        StaticDomTags.makeClassTag(StaticDomTags.TAG_LINEPATTERN_REF, trainrunTimeCategory.linePatternRef) +
        ' ' + StaticDomTags.TAG_SELECTED;
    }
    return 'TrainrunDialog TimeCategory ' +
      StaticDomTags.makeClassTag(StaticDomTags.TAG_LINEPATTERN_REF, trainrunTimeCategory.linePatternRef);
  ...
  onTimeCategoryChanged(trainrunTimeCategory: TrainrunTimeCategory) {
    ...
    this.trainrunService.updateTrainrunTimeCategory(this.selectedTrainrun, this.selectedTimeCategory);
  }
  ...
  onDeleteTrainrun() {
    ...
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.trainrunSectionService.deleteAllTrainrunSectionsOfTrainrun(this.selectedTrainrun.getId());
