...
export class GeneralViewFunctions {
  static calcDialogTopLeftScreenCoordinate(windowsPosition: Vec2D, dialogDimension: Vec2D): Vec2D {
    ...
    return GeneralViewFunctions.calcDialogTopLeftScreenCoordinateClientRectInfo(
      windowsPosition, dialogDimension,
      rectHtml.top, rectHtml.left, rectHtml.height, rectHtml.width
    );

  }

  static calcDialogTopLeftScreenCoordinateClientRectInfo(windowsPosition: Vec2D, dialogDimension: Vec2D,
                                                         clientTop: number, clientLeft: number,
                                                         clientHeight: number, clientWidth: number): Vec2D {
    ...
    let left: number = (windowsPosition.getX() - dialogDimension.getX() / 2 - RASTERING_BASIC_GRID_SIZE / 2);
