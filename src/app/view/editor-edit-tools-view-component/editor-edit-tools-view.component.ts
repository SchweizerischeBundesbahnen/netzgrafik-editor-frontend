...
export class EditorEditToolsViewComponent implements OnDestroy {
  ...
  @ViewChild('netzgrafikMergeFileInput', {static: false}) netzgrafikMergeFileInput: ElementRef;
  @ViewChild('netzgrafikMergeAsACopyFileInput', {static: false}) netzgrafikMergeAsACopyFileInput: ElementRef;
  ...
  onClearAllFiltered() {
    ...
    const dialogContent = 'Sollen alle nicht sichtbare Elemene aus der Netzgrafik definitiv gelöscht werden?';
    const confirmationDialogParamter = new ConfirmationDialogParameter(dialogTitle, dialogContent);
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
  ...
  onClear() {
    const dialogTitle = 'Löschen';
    const dialogContent = 'Sollen alle sichtbare Elemene aus der Netzgrafik definitiv gelöscht werden?';
    const confirmationDialogParamter = new ConfirmationDialogParameter(dialogTitle, dialogContent);
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
  ...
  onClearAllTrainruns() {
    const dialogTitle = 'Löschen';
    const dialogContent = 'Sollen alle sichtbare Züge definitiv gelöscht werden?';
    const confirmationDialogParamter = new ConfirmationDialogParameter(dialogTitle, dialogContent);
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.trainrunSectionService.deleteAllVisibleTrainrunSections();
  ...
  onClearAllNotes() {
    const dialogTitle = 'Löschen';
    const dialogContent = 'Sollen alle sichtbare Kommentare definitiv gelöscht werden?';
    const confirmationDialogParamter = new ConfirmationDialogParameter(dialogTitle, dialogContent);
    this.uiInteractionService.showConfirmationDiagramDialog(confirmationDialogParamter).subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.noteService.deleteAllVisibleNotes();
  ...
  onLoadNetzgrafikToMergeAsACopy(param) {
    ...
    this.loadNetzgrafik(param, netzgrafikDto => this.dataService.insertCopyNetzgrafikDto(netzgrafikDto));
  }

  onLoadNetzgrafikToMerge(param) {
    this.uiInteractionService.closeNodeStammdaten();
    this.uiInteractionService.closePerlenkette();
    this.loadNetzgrafik(param, netzgrafikDto => this.dataService.mergeNetzgrafikDto(netzgrafikDto));
  }
  ...
  private loadNetzgrafik(param, callback) {
    ...
    reader.onload = () => {
      ...
      if (
        ...
        this.logger.log('onLoadNetzgrafikToMerge; load netzgrafik: ', netzgrafikDto);
