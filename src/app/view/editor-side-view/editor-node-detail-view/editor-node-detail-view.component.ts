...
export class EditorNodeDetailViewComponent implements OnInit, OnDestroy {
  ...
  onConnectionTimeChanged() {
    this.nodeService.changeConnectionTime(this.nodeProperties.nodeId, this.nodeProperties.nodeConnectionTime);
  }
  ...
  remove(chipEvent: SbbChipEvent) {
    ...
    this.nodeProperties.labels = this.nodeProperties.labels.filter(labels => labels !== valueDelete);
    this.nodeService.changeLabels(this.nodeProperties.nodeId, this.nodeProperties.labels);
  }
  ...
  onCapacityChanged() {
    if (this.nodeProperties.nodeCapacity > 0) {
      this.resourceService.changeCapacity(this.nodeProperties.nodeResouceId, this.nodeProperties.nodeCapacity);
    } else {
  ...
  loadCapacityValue() {
    this.nodeProperties.nodeCapacity
      = this.resourceService.getResource(this.nodeProperties.nodeResouceId).getCapacity();
  }

  onDeleteNode() {
    const node = this.nodeService.getSelectedNode();
    const dialogTitle = 'Löschen';
    const dialogContent = 'Soll der Knoten ' + node.getBetriebspunktName() + ' (' + node.getFullName() + ') definitiv gelöscht werden?';
  ...
  onHaltezeitChanged() {
    this.nodeService.changeHaltezeit(this.nodeProperties.nodeId, this.nodeProperties.nodeTrainrunCategoryHaltezeit);
  }
