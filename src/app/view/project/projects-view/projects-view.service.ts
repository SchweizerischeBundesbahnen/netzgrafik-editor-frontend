...
import {ProjectControllerBackendService, ProjectCreateUpdateDto, ProjectSummaryDto} from '../../../api/generated';
...
export class ProjectsViewService implements OnDestroy {
  ...
  constructor(private readonly projectService: ProjectControllerBackendService) {
    this.filteredProjects = this.constructFilteredProjects().pipe(
        map(projectSummaryDto => projectSummaryDto
          .sort(this.sortByProjectSummaryDtoName)
        )
    );
    this.loadProjects();
  }
  ...
  private static filterProject(searchQuery: string, showArchive: boolean, project: ProjectSummaryDto) {
    if (!searchQuery) {
      return project.isArchived === showArchive;
    }
    return project.isArchived === showArchive && project.name.toLocaleLowerCase().includes(searchQuery);
  }
  ...
  private constructFilteredProjects() {
    return combineLatest([this.searchQuery, this.showArchive, this.allProjects])
      .pipe(map(([search, showArchive, projects]) =>
