...
describe('Theme Tests', () => {
  ...
  it('ThemeBase test - extended', () => {
    expect((new ThemeBase(ThemeRegistration.ThemeDefaultUx, 'red', 'white', false))
      .isDark).toBe(false);
    expect((new ThemeBase(ThemeRegistration.ThemeDefaultUxDark, 'red', 'white', false))
      .isDark).toBe(false);
    expect((new ThemeBase(ThemeRegistration.ThemeFach, 'red', 'white', false))
      .isDark).toBe(false);
    expect((new ThemeBase(ThemeRegistration.ThemeFachDark, 'red', 'white', false))
      .isDark).toBe(false);
    expect((new ThemeBase(ThemeRegistration.ThemeFachPrint, 'red', 'white', true))
      .isDark).toBe(true);
    expect((new ThemeBase(ThemeRegistration.ThemeGray, 'red', 'white', true))
      .isDark).toBe(true);
    expect((new ThemeBase(ThemeRegistration.ThemeGrayDark, 'red', 'white', true))
      .isDark).toBe(true);
  });
  ...
  it('check...Colors_keys() ', () => {
    const nbrColorEntries = 46;
    const keys: string[ ] = [];
    ThemeDefaultUx.getThemeDefaultUxColors().forEach(
      (str: string) => keys.push(str.split(':')[0])
    );
  ...
  it('ThemeBase..GlobalColors ', () => {
    ...
    expect(ThemeBase.getGlobalColors('red', 'yellow')
      .includes('COLOR_BACKGROUND: red')).toBe(true);
    expect(ThemeBase.getGlobalColors('red', 'yellow')
      .includes('COLOR_STRECKENGRAPHIK_BACKGROUND: yellow')).toBe(true);
  });
