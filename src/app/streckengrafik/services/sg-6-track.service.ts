...
export class Sg6TrackService implements OnDestroy {
  ...
  constructor(private readonly sg5FilterService: Sg5FilterService,
    ...
    this.sg5FilterService.getSgSelectedTrainrun()
      .pipe(takeUntil(this.destroyed$))
      .subscribe(selectedTrainrun => {
        this.maxFrequency = 0;
        this.dataService.getNetzgrafikDto().metadata.trainrunFrequencies.forEach((freq: TrainrunFrequency) => {
  ...
  private render() {
    ...
    const sectionTrackMap = this.extractStreckenGleis(this.selectedTrainrun.trainruns, separateForwardBackwardSectionTracks);
    this.mapTrackToTop(this.selectedTrainrun.trainruns, sectionTrackMap, separateForwardBackwardSectionTracks);
  ...
  private extractStreckenGleis(trainrunItems: SgTrainrun[],
    ...
    const sectionsOfInterest = this.createSectionsOfInterestMap(trainrunItems, separateForwardBackwardTracks);
    return this.extractSectionTracks(sectionsOfInterest);
  }

  private createSectionsOfInterestMap(trainrunItems: SgTrainrun[], separateForwardBackwardTracks: boolean) {
    /*
  ...
  private getSectionKey(ps: SgTrainrunSection,
    ...
    let key1 = (ps.arrivalPathNode === undefined ? ('undefined_' + separateForwardBackwardTracks) : ps.arrivalPathNode.nodeShortName) + '_' + node1 + '_' + ps.index;
    let key2 = (ps.departurePathNode === undefined ? ('undefined_' + separateForwardBackwardTracks) : ps.departurePathNode.nodeShortName) + '_' + node2 + '_' + ps.index;
    if (node1 > node2) {
      const tmp = node1;
      node1 = node2;
      node2 = tmp;
  ...
  private mergeDistanceCellGridResoultionToBigSegment(tracksMatrix: number[], nDistanceCells: number): any[] {
  ...
  private extractSectionTracks(sectionsOfInterest: Map<string, any[]>,
                               distRes = 4, // Res 15s
                               timeRes = 4 // Res 15s
  ) {
    ...
    for (const keyNodeId of sectionsOfInterest.keys()) {
      ...
      sectionData.forEach((d) => {
        ...
        const ts = this.trainrunSectionService.getTrainrunSectionFromId(item.getTrainrunSection().trainrunSectionId);
        const headwayTime = (ts !== undefined) ? ts.getTrainrun().getTrainrunCategory().sectionHeadway : this.minimumHeadwayTime;

        // iterate cell-by-cell foward
        for (let distCellIdx = 0; distCellIdx < nDistanceCells; distCellIdx++) {
          // unroll frequency to get the trains - generated out of the "template" train
          for (let freqLoop = 0; freqLoop <= this.maxFrequency; freqLoop = freqLoop + d.trainrun.frequency) {
            ...
            for (let bandOffset = 0; bandOffset < timeRes * headwayTime; bandOffset++) {
              ...
              let timeCellIdx = (item.departureTime % this.maxFrequency) + travelTime * distCellIdx / (nDistanceCells - 1) + freqLoop;
      ...
      // ------------------------------------------------------------------------------------------------------------------
      sectionsTracks.set(keyNodeId, this.mergeDistanceCellGridResoultionToBigSegment(tracksMatrix, nDistanceCells));
    }
  ...
  private getOrCreateTrackAlignmentsNodeData(nodesOfInterest: Map<string, // node id (key)
                                               Map<number, // track (key)
                                                 [number, number][]>>,
                                             pn: SgTrainrunNode) {
  ...
  private checkOccupancyTrackAlignments(trackData: [number, number][],
                                        item: SgTrainrunItem,
                                        arrTime: number,
                                        depTime: number,
                                        ts: SgTrainrun,
                                        minimumHeadwayTime: number): boolean {
    const arrDepTimes =
      trackData.sort((a, b) => a[0] < b[0] ? -1 : 1);
    const depArrTimes =
      trackData.sort((a, b) => a[1] < b[1] ? -1 : 1);

    for (let offset = -((4 * this.maxFrequency) / ts.frequency); offset < ((4 * this.maxFrequency) / ts.frequency); offset++) {
      if (item.checkUnrollAllowed(offset)) {
        if (!this.checkOccupancyTrackElement(arrTime, depTime, offset, arrDepTimes, depArrTimes, ts, minimumHeadwayTime)) {
          return true;
        }
      }
    }
  ...
  private checkOccupancyTrackElement(arrTime: number,
                                     depTime: number,
                                     offset: number,
  ...
  private extractTrainrunTrackAlignments(trackData: [number, number][],
                                         item: SgTrainrunItem,
                                         arrTime: number,
                                         depTime: number,
                                         ts: SgTrainrun,
  ...
  private transformUmlaufNodePair(forwardNode: SgTrainrunNode,
  ...
  private transformTurnarounds(item: SgTrainrunNode, trainrun: SgTrainrun, minimumHeadwayTime: number) {
    if (!item.endNode) {
      return;
    }
    ...
    trainrun.sgTrainrunItems.filter(el => el.isNode()).forEach(el => {
    ...
    if (!item.backward && item.index === minIndexEndNode) {
      ...
      const backwardNode = trainrun.sgTrainrunItems.find(el => el.backward && el.index === item.index).getTrainrunNode();
    ...
    if (item.backward && item.index !== minIndexEndNode) {
      ...
      const forwardNode = trainrun.sgTrainrunItems.find(el => !el.backward && el.index === item.index).getTrainrunNode();
  ...
  private detectAndCreateExtraTrains(ts: SgTrainrun, trainrunNode: SgTrainrunNode, minimumHeadwayTime: number, specialItems: SgTrainrunNode[]) {
  ...
  private computeTrackAlignments(selectedTrainrun: SgSelectedTrainrun,
    ...
    const nodesOfInterest =
      new Map<string, // node id (key)
        Map<number, // track (key)
          [number, number][]>>(); //   arrTimes / depTimes

    // ------------------------------------------------------------------------------------------------------------------
    // process for each trainrun the three pass pipeline
    // ------------------------------------------------------------------------------------------------------------------
    selectedTrainrun.trainruns.forEach(
      ts => {
        ...
        ts.sgTrainrunItems.forEach(item => {
          if (item.isNode()) {
            this.detectAndCreateExtraTrains(ts, item.getTrainrunNode(), item.minimumHeadwayTime, collectExtraTrainruns);
          }
        });
        ...
        ts.sgTrainrunItems.forEach(item => {
          if (item.isNode()) {
            ...
            if (pn.endNode && !pn.unusedForTurnaround) {
              this.alignTrainrunNodeToTrack(item, ts, nodesOfInterest, trackInfoMap, item.minimumHeadwayTime, separateForwardBackwardTracks);
            }
          }
        });
  ...
  private alignTrainrunNodeToTrack(item: SgTrainrunItem,
                                   ts: SgTrainrun,
                                   nodesOfInterest: Map<string, // node id (key)
                                     Map<number, // track (key)
                                       [number, number][]>>,
    ...
    let trackIdx = 0;
    let trackOfInterest = this.estimateTrackOfInterest(trackIdx, item, separateForwardBackwardTracks, trackInfoMap);
    ...
    while (isOccupied) {
      ...
      if (separateForwardBackwardTracks && !pn.endNode) {
        if (item.backward) {
          if (trackOfInterest % 2 === 0) {
            isOccupied = this.checkOccupancyTrackAlignments(trackData, item, item.arrivalTime, item.departureTime, ts, minimumHeadwayTime);
          }
        } else {
      ...
      if (isOccupied) {
        trackIdx += 1;
        trackOfInterest = this.estimateTrackOfInterest(trackIdx, item, separateForwardBackwardTracks, trackInfoMap);
    ...
    nodeData.set(trackOfInterest, this.extractTrainrunTrackAlignments(trackData, item, item.arrivalTime, item.departureTime, ts, minimumHeadwayTime));
    nodesOfInterest.set(this.getNodeKeyAlignmentsNodeData(pn), nodeData);
  }
  ...
  private makeTrackSymmetricUpdateTrackAlignment(tracks: PathNodeNeighbour[], separateForwardBackwardTracks: boolean) {
    let trackItr = 0;
    let prevPn = tracks[0];
    let mainTrackIdx = -1;
    ...
    tracks.forEach((pn: PathNodeNeighbour) => {
      ...
      pn.pathNodes.forEach(sgTN => {
        const curTrack = this.getTrackNumber(pn.trackNbr, sgTN.backward, separateForwardBackwardTracks);
  ...
  private makeTrackSymmetric(
    ...
    for (const keyNodeId of nodeTracksMap.keys()) {
      ...
      const tracKInfo = this.makeTrackSymmetricUpdateTrackAlignment(tracks, separateForwardBackwardTracks);
      ...
      tracks.forEach((pn: PathNodeNeighbour) => {
        pn.pathNodes.forEach(sgTN => {
          if (!sgTN.unusedForTurnaround) {
            ...
            const offsetTrack = tracKInfo.mainTrackOffsetMap.get('' + pn.nodeId1 + '#' + pn.nodeId2);
            const curTrack =
              ((pn.mainTrackIdx % 2 === 0) ?
  ...
  private estimateTrackOfInterest(trackIdx: number,
                                  trainrunItem: SgTrainrunItem,
                                  separateForwardBackwardTracks: boolean,
                                  nodeTracksMap: Map<number, PathNodeNeighbour[]>): number {
    ...
    if (nodeTracks === undefined) {
      ...
      return this.getTrackNumber(nodeNeighbors.trackNbr, trainrunItem.backward, separateForwardBackwardTracks);
    }
    ...
    return this.getTrackNumber(trackData.trackNbr, trainrunItem.backward, separateForwardBackwardTracks);
  }
  ...
  private getTrackNumber(track: number,
                         backward: boolean,
                         separateForwardBackwardTracks: boolean): number {
    return track;
  }

  private getConnectNeighbourNodeTracks(nodeTracks: PathNodeNeighbour[],
                                        nodeNeighbors: PathNodeNeighbour): PathNodeNeighbour[] {
    return nodeTracks.filter((pnn: PathNodeNeighbour) => {
      ...
      return pnn.nodeId1 === nodeNeighbors.nodeId1 && pnn.nodeId2 === nodeNeighbors.nodeId2 ||
  ...
  private mapTrackToTop(trainrunItems: SgTrainrun[],
        ...
        if (pathItem.isNode()) {
          if (maxTrackMap.has('' + pathItem.getTrainrunNode().nodeId)) {
            ...
            if (trackData.track < pathItem.getTrainrunNode().trackData.track) {
              maxTrackMap.set('' + pathItem.getTrainrunNode().nodeId, pathItem.getTrainrunNode().trackData);
            }
          } else {
            maxTrackMap.set('' + pathItem.getTrainrunNode().nodeId, pathItem.getTrainrunNode().trackData);
          }
        ...
        if (pathItem.isSection()) {
          ...
          if (ps.trainrunBranchType === TrainrunBranchType.Trainrun) {
            ...
            if (trackSegements !== undefined) {
              const convertedTrackSegments: TrackSegments[] = this.convertTrackSegments(trackSegements, false, 1);
              let maxTracks = 0;
              ...
              maxTrackMap.set(pathItem.getTrainrunSection().departureNodeId + ':' + pathItem.getTrainrunSection().arrivalNodeId, ps.trackData);
            }
          }
        }
      });
  ...
  private setMaxTrackMap(maxTrackMap: Map<string, TrackData>) {
    maxTrackMap.forEach((trackData, key) => {
      this.selectedTrainrun.paths.forEach(path => {
        ...
        if (path.isSection()) {
          if (path.getPathSection().arrivalNodeId + ':' + path.getPathSection().departureNodeId === key) {
...
class PathNodeNeighbour {
  constructor(public nodeId1: number,
