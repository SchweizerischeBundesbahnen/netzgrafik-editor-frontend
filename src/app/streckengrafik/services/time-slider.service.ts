...
export class TimeSliderService implements OnDestroy {
  ...
  private readonly sliderChangeSubject = new BehaviorSubject<SliderChangeInfo>(new SliderChangeInfo(this.yZoom, this.yMove));
  ...
  constructor(
    ...
    drawingBackgroundMouseListenerSerice.getMouseMoveObservable()
  ...
  handleWheelZoom(zoomingPoint: number, zoomingDelta: number, timeStamp: number) {
    ...
    let yZoom =
      this.getYZoom() - zoomingDelta * 0.005 * (1.0 + 0.1 * Math.exp(-(1.0 / (this.minZoom + this.getYZoom()))));
