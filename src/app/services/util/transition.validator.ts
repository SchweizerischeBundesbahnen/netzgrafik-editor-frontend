...
export class TransitionValidator {
  static validateTransition(node: Node, transitionId: number) {
    if (!node.getIsNonStop(transitionId)) {
      ...
      const arrivalTime1 = node.getArrivalTime(
        trainrunSections.trainrunSection1,
      );
      const departureTime1 = node.getDepartureTime(
        trainrunSections.trainrunSection1,
      );

      const arrivalTime2 = node.getArrivalTime(
        trainrunSections.trainrunSection2,
      );
      const departureTime2 = node.getDepartureTime(
        trainrunSections.trainrunSection2,
      );
