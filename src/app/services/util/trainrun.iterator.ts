...
export class TrainrunIterator {
  ...
  public next(): TrainrunSectionNodePair {
    this.currentElement = Object.assign({}, this.pointerElement);
    const trainrunSection = this.pointerElement.node.getNextTrainrunSection(this.pointerElement.trainrunSection);
    ...
    if (this.visitedNodes.find((element) => element.node.getId() === node.getId() &&
      element.trainrunSection.getId() === trainrunSection.getId()) !== undefined) {
      // The trainrun has a loop -> early break the avoid unfinitiy iterating
      this.currentElement = Object.assign({}, this.pointerElement);
      this.pointerElement = new TrainrunSectionNodePair(undefined, undefined);
      // log the issue
      this.logService.error('Iterator has detected an infinity loop. The iteration terminated early!', new Error().stack);
      return this.currentElement;
    }
    this.visitedNodes.push(this.currentElement);
