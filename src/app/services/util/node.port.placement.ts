  ...
  static placePortsOnSourceAndTargetNode(sourceNode: Node, targetNode: Node): NodePortPlacement {
    ...
    if (VisAVisPortPlacement.drawVerticalLine(posStart, posEnd)) {
      if (posStart.getY() < posEnd.getY()) {
        return {sourcePortPlacement: PortAlignment.Bottom, targetPortPlacement: PortAlignment.Top};
      } else {
        return {sourcePortPlacement: PortAlignment.Top, targetPortPlacement: PortAlignment.Bottom};
      }

    } else {
      if (posStart.getX() < posEnd.getX()) {
        return {sourcePortPlacement: PortAlignment.Right, targetPortPlacement: PortAlignment.Left};
      } else {
        return {sourcePortPlacement: PortAlignment.Left, targetPortPlacement: PortAlignment.Right};
      }
    }
  }
}
