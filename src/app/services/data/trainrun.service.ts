...
export class TrainrunService {
  ...
  setTrainrunData(trainrunsDto: TrainrunDto[]) {
    this.trainrunsStore.trainruns = trainrunsDto.map(trainrunDto => {
      ...
      trainrun.setTrainrunTimeCategory(this.dataService.getTrainrunTimeCategory(trainrunDto.trainrunTimeCategoryId));
      return trainrun;
    });
  }
  ...
  mergeTrainruns(trainruns: TrainrunDto[]): Map<number, number> {
    const trainrunMap = new Map<number, number>();
    trainruns.forEach(trainrun => {
      const equalTrainrun = this.trainrunsStore.trainruns.find(tr =>
  ...
  getSelectedOrNewTrainrun(): Trainrun {
    ...
    if (trainrun === undefined) {
      ...
      trainrun.setTrainrunCategory(this.dataService.getTrainrunCategory(Trainrun.DEFAULT_TRAINRUN_CATEGORY));
      trainrun.setTrainrunFrequency(this.dataService.getTrainrunFrequency(Trainrun.DEFAULT_TRAINRUN_FREQUENCY));
      trainrun.setTrainrunTimeCategory(this.dataService.getTrainrunTimeCategory(Trainrun.DEFAULT_TRAINRUN_TIME_CATEGORY));
      trainrun.select();
  ...
  deleteTrainrun(trainrun: Trainrun, enforceUpdate = true) {
    const deletetLabelIds = this.labelService.clearLabel(trainrun.getLabelIds(), this.makeLabelIDCounterMap(this.getTrainruns()));
    this.filterService.clearDeletetFilterTrainrunLabels(deletetLabelIds);
    this.trainrunsStore.trainruns.forEach(tr => tr.unselect());
    this.trainrunsStore.trainruns = this.trainrunsStore.trainruns.filter(tr => tr.getId() !== trainrun.getId());
    if (enforceUpdate){
  ...
  updateTrainrunFrequency(trainrun: Trainrun, frequency: TrainrunFrequency, offset: number): number {
    ...
    this.trainrunSectionService.getAllTrainrunSectionsForTrainrun(trainrun.getId()).forEach(
      (ts: TrainrunSection) => {
  ...
  getAllTrainrunLabels(): string[] {
    ...
    this.getTrainruns().forEach((t) =>
      this.labelService.getTextLabelsFromIds(t.getLabelIds()).forEach(label => trainrunLabels.push(label))
    );
  ...
  visibleTrainrunsDeleteLabel(labelRef: string) {
    ...
    const trainruns = this.getTrainruns().find(
      (t: Trainrun) => (t.getLabelIds().find((labelId: number) => labelId === labelObject.getId()) !== undefined)
    );
  ...
  getConnectedTrainrunIdsFirstOrder(trainrunId: number): number[] {
    const trainrunSections = this.trainrunSectionService.getAllTrainrunSectionsForTrainrun(trainrunId);
    let connectedTrainrunIds: number[] = [];
    for (const trainsection of trainrunSections) {
      connectedTrainrunIds = connectedTrainrunIds
        .concat(trainsection.getSourceNode().getAllConnectedTrainruns(trainsection));
      connectedTrainrunIds = connectedTrainrunIds
        .concat(trainsection.getTargetNode().getAllConnectedTrainruns(trainsection));
    }
  ...
  duplicateTrainrun(trainrunId: number, enforceUpdate = true, postfix = ' COPY'): Trainrun {
    ...
    this.trainrunSectionService.copyAllTrainrunSectionsForTrainrun(trainrun.getId(), copiedtrainrun.getId());
  ...
  setLabels(trainrunId: number, labels: string[]) {
    ...
    labels.forEach(label => {
      labelIds.push(this.labelService.getOrCreateLabel(label, LabelRef.Trainrun).getId());
    });
    const deletetLabelIds = this.labelService.clearLabel(this.findClearedLabel(trainrun, labelIds),
  ...
  propagateConsecutiveTimesForTrainrun(trainrunSectionId: number) {
    ...
    const bothEndNodes = this.getBothEndNodesWithTrainrunId(trainrunSection.getTrainrunId());
    const startForwardNode = GeneralViewFunctions.getLeftOrTopNode(bothEndNodes.endNode1, bothEndNodes.endNode2);
    const startBackwardNode = bothEndNodes.endNode1.getId() === startForwardNode.getId() ? bothEndNodes.endNode2 : bothEndNodes.endNode1;
    const arrivalTime = this.propagateConsecutiveTimes(startForwardNode,
    ...
    this.propagateConsecutiveTimes(startBackwardNode,
      startBackwardNode.getStartTrainrunSection(trainrunSection.getTrainrunId()), offset);
  }
  ...
  getCumulativeTravelTime(trainrunSection: TrainrunSection) {
    ...
    return this.sumTravelTimeUpToLastNonStopNode(iterator.current().node, iterator.current().trainrunSection);
  }
  ...
  getCumulativeTravelTimeAndNodePath(trainrunSection: TrainrunSection) {
    ...
    return this.getCumSumTravelTimeNodePathToLastNonStopNode(iterator.current().node, iterator.current().trainrunSection);
  }
  ...
  private createNewTrainrunFromDto(trainrun: TrainrunDto): Trainrun {
    ...
    newTrainrun.setTrainrunTimeCategory(this.dataService.getTrainrunTimeCategory(trainrun.trainrunTimeCategoryId));
  ...
  private propagateConsecutiveTimes(node: Node, trainrunSection: TrainrunSection, offset: number): number {
    if (trainrunSection === undefined) {
      return 0;
    }
    ...
    while (iterator.hasNext()) {
      const nextPair = iterator.next();
      nextPair.node.getOppositeNode(nextPair.trainrunSection)
        .setDepartureConsecutiveTime(nextPair.trainrunSection, accumulatedTime);

      const oppositeNodeDepartureTime =
        nextPair.node.getOppositeNode(nextPair.trainrunSection).getDepartureTime(nextPair.trainrunSection);
      ...
      if (!nextPair.node.isEndNode(nextPair.trainrunSection)) {
        ...
        halteZeit = nextDeparture < oldArrival ?
