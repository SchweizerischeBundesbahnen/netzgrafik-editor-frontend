...
import {SymmetryOn, SymmetryReference} from "../../view/dialogs/symmetry-selection-dialog/symmetry-selection-dialog.component";
...
export class TrainrunSectionTimesService {
  ...
  onNodeLeftDepartureTimeChanged() {
    ...
    if (!this.lockStructure.rightLock) {
      this.timeStructure.rightArrivalTime = this.timeStructure.leftDepartureTime + (this.timeStructure.travelTime % 60);
      ...
      if (this.isLeftNodeSymmetric()) {
        ...
        if (this.lockStructure.leftLock && !this.lockStructure.travelTimeLock) {
          this.timeStructure.bottomTravelTime = this.timeStructure.leftArrivalTime - this.timeStructure.rightDepartureTime;
          this.timeStructure.bottomTravelTime += this.timeStructure.bottomTravelTime <= 0 ? 60 : 0;
        } else {
          this.timeStructure.leftArrivalTime = this.timeStructure.rightDepartureTime + (this.timeStructure.bottomTravelTime % 60);
      ...
      this.timeStructure.travelTime = this.timeStructure.rightArrivalTime - this.timeStructure.leftDepartureTime;
  ...
  onNodeLeftArrivalTimeChanged() {
    ...
    if (!this.lockStructure.rightLock) {
      this.timeStructure.rightDepartureTime = this.timeStructure.leftArrivalTime - (this.timeStructure.bottomTravelTime % 60);
      ...
      if (!this.isLeftNodeSymmetric()) {
        if (this.lockStructure.leftLock) {
          ...
          this.timeStructure.leftDepartureTime = this.timeStructure.rightArrivalTime - (this.timeStructure.travelTime % 60);
  ...
  onTravelTimeChanged() {
    ...
    if (!this.lockStructure.rightLock) {
      this.timeStructure.rightArrivalTime = this.timeStructure.leftDepartureTime + this.timeStructure.travelTime;
      ...
      if (!this.selectedTrainrunSection.isSymmetric()) {
        if (this.lockStructure.leftLock) {
          ...
          this.timeStructure.leftArrivalTime = this.timeStructure.rightDepartureTime + this.timeStructure.bottomTravelTime;
      ...
      this.timeStructure.leftDepartureTime = this.timeStructure.rightArrivalTime - this.timeStructure.travelTime;
      this.timeStructure.leftDepartureTime += this.timeStructure.leftDepartureTime < 0 ? 60 : 0;
      this.timeStructure.leftDepartureTime %= 60;
      if (this.isLeftNodeSymmetric()) {
        this.timeStructure.leftArrivalTime = TrainrunsectionHelper.getSymmetricTime(this.timeStructure.leftDepartureTime);
  ...
  onBottomTravelTimeChanged() {
    ...
    if (!this.lockStructure.leftLock) {
      ...
      this.timeStructure.rightDepartureTime = this.timeStructure.leftArrivalTime - this.timeStructure.bottomTravelTime;
      this.timeStructure.rightDepartureTime += this.timeStructure.rightDepartureTime < 0 ? 60 : 0;
      this.timeStructure.rightDepartureTime %= 60;
      if (this.isRightNodeSymmetric()) {
        this.timeStructure.rightArrivalTime = TrainrunsectionHelper.getSymmetricTime(this.timeStructure.rightDepartureTime);
  ...
  /* Symmetry */
  onLeftNodeSymmetryChanged(isLeftNodeSymmetric: boolean, isPositionSwapped: boolean, reference: SymmetryReference = null) {
    ...
    this.removeOffsetAndBackTransformTimeStructure();
    this.timeStructure = this.calculateTimeStructureAfterSymmetrySelection(SymmetryOn.LeftNode, reference);
    this.updateTrainrunSectionTime();
    this.applyOffsetAndTransformTimeStructure();
  }

  onRightNodeSymmetryChanged(isRightNodeSymmetric: boolean, isPositionSwapped: boolean, reference: SymmetryReference = null) {
    if (isPositionSwapped) {
      this.trainrunSectionService.updateSourceSymmetry(
        this.selectedTrainrunSection.getId(),
    ...
    this.removeOffsetAndBackTransformTimeStructure();
    this.timeStructure = this.calculateTimeStructureAfterSymmetrySelection(SymmetryOn.RightNode, reference);
    this.updateTrainrunSectionTime();
    this.applyOffsetAndTransformTimeStructure();
  }
  ...
  onTrainrunSymmetryChanged(trainrunId: number, reference: SymmetryReference) {
    this.trainrunSectionService.getAllTrainrunSectionsForTrainrun(trainrunId).forEach((section) => {
      ...
      this.updateTrainrunSectionTime(
        section,
        this.calculateTimeStructureAfterSymmetrySelectionForTrainrunSection(
          reference,
          section,
        ),
      );
  ...
  getTrainrunLeftAndRightTimeStructure(): Omit<LeftAndRightTimeStructure, "travelTime" | "bottomTravelTime"> {
    const {firstTrainrunSection, lastTrainrunSection, swapped} = this.trainrunService.getFirstAndLastTrainrunSections(
      this.selectedTrainrunSection.getTrainrunId()
    );
    return {
      leftDepartureTime: swapped ? firstTrainrunSection.getTargetDeparture() : firstTrainrunSection.getSourceDeparture(),
      leftArrivalTime: swapped ? firstTrainrunSection.getTargetArrival() : firstTrainrunSection.getSourceArrival(),
      rightDepartureTime: swapped ? lastTrainrunSection.getSourceDeparture() : lastTrainrunSection.getTargetDeparture(),
      rightArrivalTime: swapped ? lastTrainrunSection.getSourceArrival() : lastTrainrunSection.getTargetArrival(),
    };
  }

  calculateTimeStructureAfterSymmetrySelection(symmetryOn: SymmetryOn, reference: SymmetryReference): LeftAndRightTimeStructure {
    const timeStructure = Object.assign({}, this.timeStructure);
    switch (symmetryOn) {
      case SymmetryOn.LeftNode: {
        if (reference === SymmetryReference.Top) {
          timeStructure.leftArrivalTime = TrainrunsectionHelper.getSymmetricTime(timeStructure.leftDepartureTime);
        } else {
          timeStructure.leftDepartureTime = TrainrunsectionHelper.getSymmetricTime(timeStructure.leftArrivalTime);
        }
        if (this.isRightNodeSymmetric()) {
          if (reference === SymmetryReference.Top) {
          ...
          timeStructure.rightArrivalTime = timeStructure.leftDepartureTime + timeStructure.travelTime;
          timeStructure.rightArrivalTime %= 60;
          timeStructure.rightDepartureTime = TrainrunsectionHelper.getSymmetricTime(timeStructure.rightArrivalTime);
        } else {
          if (!this.lockStructure.rightLock) {
            timeStructure.rightDepartureTime = timeStructure.leftArrivalTime - timeStructure.bottomTravelTime;
            ...
            timeStructure.travelTime = timeStructure.rightArrivalTime - timeStructure.leftDepartureTime;
            timeStructure.travelTime += timeStructure.travelTime <= 0 ? 60 : 0;
            timeStructure.bottomTravelTime = timeStructure.leftArrivalTime - timeStructure.rightDepartureTime;
      ...
      case SymmetryOn.RightNode: {
        if (reference === SymmetryReference.Top) {
          ...
          timeStructure.rightArrivalTime = TrainrunsectionHelper.getSymmetricTime(timeStructure.rightDepartureTime);
        }
        if (this.isLeftNodeSymmetric()) {
          if (reference === SymmetryReference.Top) {
            timeStructure.bottomTravelTime = timeStructure.travelTime;
          ...
          timeStructure.leftArrivalTime = timeStructure.rightDepartureTime + timeStructure.bottomTravelTime;
          timeStructure.leftArrivalTime %= 60;
          timeStructure.leftDepartureTime = TrainrunsectionHelper.getSymmetricTime(timeStructure.leftArrivalTime);
        } else {
          if (!this.lockStructure.leftLock) {
            timeStructure.leftDepartureTime = timeStructure.rightArrivalTime - timeStructure.travelTime;
      ...
      case SymmetryOn.Trainrun: {
        ...
        timeStructure.leftArrivalTime = swapped ? firstTrainrunSection.getTargetArrival() : firstTrainrunSection.getSourceArrival();
        timeStructure.leftDepartureTime = swapped ? firstTrainrunSection.getTargetDeparture() : firstTrainrunSection.getSourceDeparture();
        timeStructure.rightArrivalTime = swapped ? lastTrainrunSection.getSourceArrival() : lastTrainrunSection.getTargetArrival();
        timeStructure.rightDepartureTime = swapped ? lastTrainrunSection.getSourceDeparture() : lastTrainrunSection.getTargetDeparture();
  ...
  calculateTimeStructureAfterSymmetrySelectionForTrainrunSection(
    ...
    if (reference === SymmetryReference.Top) {
      return {
        ...
        rightDepartureTime: TrainrunsectionHelper.getSymmetricTime(trainrunSection.getTargetArrival()),
        bottomTravelTime: trainrunSection.getTravelTime(),
        leftArrivalTime: TrainrunsectionHelper.getSymmetricTime(trainrunSection.getSourceDeparture()),
      };
    } else {
      return {
        leftDepartureTime: TrainrunsectionHelper.getSymmetricTime(trainrunSection.getSourceArrival()),
        travelTime: trainrunSection.getBackwardTravelTime(),
        rightArrivalTime: TrainrunsectionHelper.getSymmetricTime(trainrunSection.getTargetDeparture()),
