...
export class TrainrunSectionService implements OnDestroy {
  ...
  static computeArrivalAndDeparture(nodeArrival: number, trainrunSection: TrainrunSection, nonStop: boolean,
                                    halteZeiten: TrainrunCategoryHaltezeit): DepartureAndArrivalTimes {
    let haltezeit = halteZeiten[trainrunSection.getTrainrun().getTrainrunCategory().fachCategory].haltezeit;
    haltezeit = nonStop ? 0 : haltezeit;
    const fromDepartureTime = MathUtils.round((nodeArrival + haltezeit) % 60, 1);
    const fromArrivalTime = MathUtils.round(TrainrunsectionHelper.getSymmetricTime(fromDepartureTime), 1);
    const toArrivalTime = MathUtils.round((fromDepartureTime + trainrunSection.getTravelTime() % 60) % 60, 1);
    const toDepartureTime = MathUtils.round(TrainrunsectionHelper.getSymmetricTime(toArrivalTime), 1);
  ...
  private static setToNode(sourceNodeId: number, trainrunSection: TrainrunSection, nodeToNew: Node, targetNodeId: number) {
  ...
  propagateTimesForNewTrainrunSection(trainrunSection: TrainrunSection) {
    ...
    if (previousTrainrunSection !== undefined) {
      ...
      const arrivalDepartureTimes = TrainrunSectionService.computeArrivalAndDeparture(previousNodeArrival,
        trainrunSection, false, halteZeit);
  ...
  getTrainrunSectionFromId(trainrunSectionId: number) {
    return this.trainrunSectionsStore.trainrunSections.find(tr => tr.getId() === trainrunSectionId);
  }
  ...
  getSelectedTrainrunSection(): TrainrunSection {
    const selectedTrainrunSection = this.trainrunSectionsStore.trainrunSections.find(tr => tr.selected());
  ...
  createNewTrainrunSectionsFromDtoList(trainrunSections: TrainrunSectionDto[],
                                       nodeMap: Map<number, number>,
                                       trainrunMap: Map<number, number>,
                                       nodes: NodeDto[]): Map<number, number> {
    const trainrunSectionMap: Map<number, number> = new Map<number, number>();
    trainrunSections.forEach(trainrunSection => {
      const newTrainrunSection = this.createNewTrainrunSectionFromDto(trainrunSection, nodeMap, trainrunMap, nodes);
  ...
  mergeTrainrunSections(trainrunSections: TrainrunSectionDto[],
                        nodeMap: Map<number, number>,
                        trainrunMap: Map<number, number>,
                        nodes: NodeDto[]) {
    trainrunSections.forEach(trainrunSection => {
      const trainrunId = trainrunMap.get(trainrunSection.trainrunId);
      const existingTrainrunSection = this.trainrunSectionsStore.trainrunSections.find((ts: TrainrunSection) =>
  ...
  initializeTrainrunSectionsWithReferencesToNodesAndTrainruns() {
    this.trainrunSectionsStore.trainrunSections.forEach(
      trainrunSection => trainrunSection.setSourceAndTargetNodeReference(
    ...
    this.trainrunSectionsStore.trainrunSections.forEach(trainrunSection => trainrunSection.setTrainrun(
  ...
  updateTrainrunSectionTime(trsId: number,
  ...
  private propagateTimeAlongTrainrunFixStartTrainrunSection(trainrunSection: TrainrunSection, node: Node, stopNodeId: number) {
  ...
  propagateTimeAlongTrainrun(trainrunSectionId: number, fromNodeIdOn: number) {
    ...
    const fromNode = fromNodeIdOn === trainrunSection.getSourceNodeId() ? trainrunSection.getSourceNode() : trainrunSection.getTargetNode();
    if (fromNode.getId() !== fromNodeIdOn) {
      let trainrunSectionIdToStart = this.propagateTimeAlongTrainrunFixStartTrainrunSection(trainrunSection, trainrunSection.getSourceNode(), fromNodeIdOn);
      if (trainrunSectionIdToStart === undefined) {
        trainrunSectionIdToStart = this.propagateTimeAlongTrainrunFixStartTrainrunSection(trainrunSection, trainrunSection.getTargetNode(), fromNodeIdOn);
      }
      if (trainrunSectionIdToStart !== undefined) {
        this.iterateAlongTrainrunUntilEndAndPropagateTime(this.nodeService.getNodeFromId(fromNodeIdOn), trainrunSectionIdToStart);
        this.trainrunSectionsUpdated();
        return;
      }
    }
  ...
  iterateAlongTrainrunUntilEndAndPropagateTime(nodeFrom: Node, trainrunSectionId: number) {
    ...
    while (iterator.hasNext()) {
      ...
      const arrivalDepartureTimes = TrainrunSectionService.computeArrivalAndDeparture(previousNodeArrival,
        pair.trainrunSection, isNonStop, halteZeit);
      if (pair.trainrunSection.getSourceNodeId() === previousPair.node.getId()) {
        if (!pair.trainrunSection.getSourceDepartureLock()) {
          pair.trainrunSection.setSourceDeparture(arrivalDepartureTimes.nodeFromDepartureTime);
          pair.trainrunSection.setSourceArrival(arrivalDepartureTimes.nodeFromArrivalTime);
          TrainrunsectionValidator.validateOneSection(pair.trainrunSection);

          if (!pair.trainrunSection.getTargetDepartureLock()) {
            pair.trainrunSection.setTargetDeparture(arrivalDepartureTimes.nodeToDepartureTime);
            pair.trainrunSection.setTargetArrival(arrivalDepartureTimes.nodeToArrivalTime);
            TrainrunsectionValidator.validateOneSection(pair.trainrunSection);
          }
        }
        ...
        if (!pair.trainrunSection.getTargetDepartureLock()) {
          pair.trainrunSection.setTargetDeparture(arrivalDepartureTimes.nodeFromDepartureTime);
          pair.trainrunSection.setTargetArrival(arrivalDepartureTimes.nodeFromArrivalTime);
          TrainrunsectionValidator.validateOneSection(pair.trainrunSection);

          if (!pair.trainrunSection.getSourceDepartureLock()) {
            pair.trainrunSection.setSourceDeparture(arrivalDepartureTimes.nodeToDepartureTime);
            pair.trainrunSection.setSourceArrival(arrivalDepartureTimes.nodeToArrivalTime);
            TrainrunsectionValidator.validateOneSection(pair.trainrunSection);
          }
        }
      }

      if (pair.trainrunSection.getSourceDepartureLock() || pair.trainrunSection.getTargetDepartureLock()) {
        break;
      }
      previousPair = pair;
    }
  ...
  updateTrainrunSectionTimeLock(trsId: number,
                                sourceLock: boolean,
                                targetLock: boolean,
  ...
  updateTrainrunSectionRouting(node: Node, enforceUpdate = true) {
    this.trainrunSectionsStore.trainrunSections.forEach(trainrunSection => {
      if (node.getId() === trainrunSection.getSourceNodeId() || node.getId() === trainrunSection.getTargetNodeId()) {
        trainrunSection.routeEdgeAndPlaceText();
      }
    });
  ...
  createTrainrunSection(sourceNodeId: number, targetNodeId: number) {
    ...
    this.logger.log('create new trainrunSection between nodes', sourceNode.getBetriebspunktName(), targetNode.getBetriebspunktName());
  ...
  reconnectTrainrunSection(sourceNodeId: number, targetNodeId: number,
    ...
    if (sourceNodeId === existingTrainrunSectionTargetNodeId ||
    ...
    if (sourceNodeId === existingTrainrunSectionSourceNodeId
      && targetNodeId === existingTrainrunSectionTargetNodeId) {
      return;
    }

    const trainrunSection: TrainrunSection = this.getTrainrunSectionFromId(existingTrainrunSectionId);

    const {
      nodeFrom,
      nodeToNew,
      nodeToOld
    } = this.getFromAndToNode(sourceNodeId, trainrunSection, targetNodeId);
    ...
    if (this.nodeService.isConditionToAddTransitionFullfilled(nodeToNew, trainrunSection)) {
      this.nodeService.addTransitionAndComputeRoutingFromFreePorts(nodeToNew, trainrunSection.getTrainrun());
    }
  ...
  deleteTrainrunSection(trainrunSectionId: number, enforceUpdate = true) {
    ...
    this.getTrainrunSections().filter((ts: TrainrunSection) =>
      ...
      .forEach((ts: TrainrunSection) => {
        this.nodeService.checkAndFixMissingTransitions(sourceNodeId, targetNodeId, ts.getId(), false);
      });
    if (enforceUpdate) {
      this.nodeService.transitionsUpdated();
      this.nodeService.connectionsUpdated();
  ...
  getDtos() {
    return this.trainrunSectionsStore.trainrunSections.map(trainrunSection => trainrunSection.getDto());
  }
  ...
  getAllTrainrunSectionsForTrainrun(trainrunID: number): TrainrunSection[] {
    const trainrunSections = this.trainrunSectionsStore.trainrunSections
      .filter((t: TrainrunSection) => t.getTrainrunId() === trainrunID);
  ...
  setTimeStructureToTrainrunSections(timeStructure: LeftAndRightTimeStructure,
    ...
    const bothLastNonStopTrainrunSections = this.trainrunService.getBothLastNonStopTrainrunSections(trainrunSection);
    const leftNode = GeneralViewFunctions.getLeftOrTopNode(bothLastNonStopNodes.lastNonStopNode1, bothLastNonStopNodes.lastNonStopNode2);
    const trs = leftNode.getId() === bothLastNonStopNodes.lastNonStopNode1.getId() ?
    ...
    while (iterator.hasNext()) {
      ...
      trsTimeStructure.travelTime = TrainrunsectionHelper.getTravelTime(newTotalTravelTime, summedTravelTime, travelTimeFactor,
        nextPair.trainrunSection.getTravelTime(), isRightNodeNonStop);
      trsTimeStructure.rightArrivalTime = TrainrunsectionHelper.getRightArrivalTime(trsTimeStructure);
      trsTimeStructure.rightDepartureTime = TrainrunsectionHelper.getRightDepartureTime(trsTimeStructure);
      const rightIsTarget = nextPair.node.getId() === nextPair.trainrunSection.getTargetNode().getId();
  ...
  trainrunSectionsUpdated() {
    this.trainrunSectionsSubject.next(Object.assign({}, this.trainrunSectionsStore).trainrunSections);
  }
  ...
  replaceIntermediateStopWithNode(trainrunSectionId: number, stopIndex: number, nodeId: number) {
    ...
    if (trainrunSection1.getSourceNodeId() === nodeId || trainrunSection1.getTargetNodeId() === nodeId) {
      return;
    }
    const origTravelTime = trainrunSection1.getTravelTime();
    const trainrunSection2 = this.copyTrainrunSection(trainrunSection1, trainrunSection1.getTrainrunId());
    ...
    this.nodeService.addTransitionToNodeForTrainrunSections(nodeIntermediate.getId(), trainrunSection1, trainrunSection2);
    ...
    const minHalteZeitFromNode = this.nodeService.getHaltezeit(nodeId, trainrunSection1.getTrainrun().getTrainrunCategory());
    let travelTime1 = trainrunSection1.getTargetDepartureConsecutiveTime() - trainrunSection1.getSourceDepartureConsecutiveTime();
    let travelTime2 = trainrunSection1.getSourceArrivalConsecutiveTime() - trainrunSection1.getTargetDepartureConsecutiveTime();
    ...
    trainrunSection1.setTargetArrival(TrainrunSectionService.boundMinutesToOneHour(
      trainrunSection1.getSourceDeparture() + trainrunSection1.getTravelTime()));
    trainrunSection2.setSourceArrival(TrainrunSectionService.boundMinutesToOneHour(
      trainrunSection1.getTargetDeparture() + trainrunSection2.getTravelTime()));
    trainrunSection1.setTargetDeparture(TrainrunSectionService.boundMinutesToOneHour(halteZeit + trainrunSection2.getSourceArrival()));
    trainrunSection2.setSourceDeparture(TrainrunSectionService.boundMinutesToOneHour(halteZeit + trainrunSection1.getTargetArrival()));

    if ((minHalteZeitFromNode < halteZeit) ||
      (trainrunSection1.getTravelTime() + trainrunSection2.getTravelTime() + halteZeit !== origTravelTime) ||
      (travelTimeIssue)) {
      const description = 'Zwischenhalteumwandlung führte bei der Vergabe der Zeiten zu inkonsistenzen!';
      trainrunSection1.setTargetArrivalWarning('Zwischenhalteumwandlung', description);
      trainrunSection1.setTargetDepartureWarning('Zwischenhalteumwandlung', description);
      trainrunSection2.setSourceArrivalWarning('Zwischenhalteumwandlung', description);
      trainrunSection2.setSourceDepartureWarning('Zwischenhalteumwandlung', description);
    }
  ...
  setWarningOnNode(trainrunSectionId: number, nodeId: number, warningTitle: string, warningDescription: string) {
    const trainrunSection = this.getTrainrunSectionFromId(trainrunSectionId);
  ...
  private copyTrainrunSection(existingTrainrunSection: TrainrunSection, newTrainrunId: number): TrainrunSection {
    const trainrunSection: TrainrunSection = new TrainrunSection();
    trainrunSection.setTrainrun(this.trainrunService.getTrainrunFromId(newTrainrunId));
    trainrunSection.setTravelTimeDto(JSON.parse(JSON.stringify(existingTrainrunSection.getTravelTimeDto())));
    trainrunSection.setSourceArrivalDto(JSON.parse(JSON.stringify(existingTrainrunSection.getSourceArrivalDto())));
    trainrunSection.setSourceDepartureDto(JSON.parse(JSON.stringify(existingTrainrunSection.getSourceDepartureDto())));
    trainrunSection.setTargetArrivalDto(JSON.parse(JSON.stringify(existingTrainrunSection.getTargetArrivalDto())));
    trainrunSection.setTargetDepartureDto(JSON.parse(JSON.stringify(existingTrainrunSection.getTargetDepartureDto())));
    trainrunSection.setNumberOfStops(existingTrainrunSection.getNumberOfStops());
    this.trainrunSectionsStore.trainrunSections.push(trainrunSection);
    return trainrunSection;
  }

  private copyTrainrunSectionAndAddToNodes(existingTrainrunSection: TrainrunSection, newTrainrunId: number): TrainrunSection {
    ...
    this.nodeService.copyTransitionProperties(sourceNode.getId(), trainrunSection, existingTrainrunSection);
    this.nodeService.copyTransitionProperties(targetNode.getId(), trainrunSection, existingTrainrunSection);
  ...
  private deleteTrainrunSectionAndCleanupNodes(trainrunSection: TrainrunSection, enforceUpdate = true) {
    ...
    this.trainrunSectionsStore.trainrunSections = this.trainrunSectionsStore.trainrunSections.filter(
      e => e.getId() !== trainrunSection.getId());
    this.reRouteAffectedTrainrunSections(trainrunSection.getSourceNodeId(), trainrunSection.getTargetNodeId());
    this.deleteTrainrunIfNotUsedAnymore(trainrunSection.getTrainrun(), false);

    if (this.getTrainrunSections().find((ts) => ts.getTrainrunId() === trainrunSection.getTrainrunId()) !== undefined) {
  ...
  private reRouteAffectedTrainrunSections(sourceNodeId: number, targetNodeId: number) {
    this.trainrunSectionsStore.trainrunSections.forEach(e => {
      if (e.getSourceNodeId() === sourceNodeId
  ...
  private getFromAndToNode(sourceNodeId: number, trainrunSection: TrainrunSection, targetNodeId: number) {
  ...
  private handleNodeAndTrainrunSectionDetails(sourceNode: Node,
                                              targetNode: Node,
                                              trainrunSection: TrainrunSection,
                                              sourceIsNonStop = false,
                                              targetIsNonStop = false) {
    this.nodeService.addPortsToNodes(sourceNode.getId(), targetNode.getId(), trainrunSection);
    this.nodeService.addTransitionToNodes(sourceNode.getId(), targetNode.getId(), trainrunSection, sourceIsNonStop, targetIsNonStop);
    this.nodeService.nodesUpdated();
    this.nodeService.transitionsUpdated();
    trainrunSection.routeEdgeAndPlaceText();
    this.reRouteAffectedTrainrunSections(sourceNode.getId(), targetNode.getId());
  ...
  private createNewTrainrunSectionFromDto(trainrunSection: TrainrunSectionDto, nodeMap: Map<number, number>,
                                          trainrunMap: Map<number, number>, nodes: NodeDto[]) {
    const trainrunId = trainrunMap.get(trainrunSection.trainrunId);
    ...
    const sourceIsNonStop = this.getIsNonStop(trainrunSection.sourceNodeId, trainrunSection.sourcePortId, nodes);
    const targetIsNonStop = this.getIsNonStop(trainrunSection.targetNodeId, trainrunSection.targetPortId, nodes);
    this.handleNodeAndTrainrunSectionDetails(sourceNode, targetNode, newTrainrunSection, sourceIsNonStop, targetIsNonStop);
  ...
  private getIsNonStop(nodeId: number, portId: number, nodes: NodeDto[]) {
    ...
    filtertNodes.forEach(node => {
      const filtertTransitions = node.transitions.filter(transition => transition.port1Id === portId || transition.port2Id === portId);
