...
export class NetzgrafikColoringService {
  ...
  netzgrafikColorStore: { netzgrafikColors: NetzgrafikColor[] } = {netzgrafikColors: []}; // store the data in memory
  ...
  private static generateLinePatternColorUI(sheet: CSSStyleSheet, verbose = false) {
    [
      'path.edge_line.layer_3.Freq_30.' + StaticDomTags.TAG_UI_DIALOG + ' {  stroke: whitesmoke; }',
      'path.edge_line.layer_2.Freq_20.' + StaticDomTags.TAG_UI_DIALOG + ' {  stroke: whitesmoke; }',
      'path.edge_line.layer_1.Freq_15.' + StaticDomTags.TAG_UI_DIALOG + ' {  stroke: whitesmoke; }',
      'path.edge_line.layer_3.Freq_15.' + StaticDomTags.TAG_UI_DIALOG + ' {  stroke: whitesmoke; }',
      'path.edge_line.layer_3.Freq_15.' + StaticDomTags.TAG_UI_DIALOG + ' {  stroke: whitesmoke; }'
    ].forEach(rule => {
  ...
  private static generateButtonColors(sheet: CSSStyleSheet, colorRef: ColorRefType, verbose = false) {
    [
      {
        tag: '',
        color: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' +
          colorRef + '_' + StaticDomTags.TAG_FOCUS.toUpperCase(),
        backgroundColor: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' +
          colorRef + '_' + StaticDomTags.TAG_MUTED.toUpperCase()
      },
      {
      ...
      const rule = 'button.' + 'ColorRef_' + colorRef + (d.tag !== '' ? '.' + d.tag : '') +
        ' { color: var(--' + d.color + '); background-color: var(--' + d.backgroundColor + '); border-color: transparent; }';
      sheet.insertRule(rule, sheet.cssRules.length);
      if (verbose) {
        console.log(rule);
      }
  ...
  private static generateColors(sheet: CSSStyleSheet, colorRef: ColorRefType, cssElement: string, domElement: string, verbose = false) {
    [
      {
        tag: '',
      ...
      },
      {
        ...
        color: StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef + '_' + StaticDomTags.TAG_FOCUS.toUpperCase()
      },
      {
        tag: StaticDomTags.TAG_MUTED,
        color: StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef + '_' + StaticDomTags.TAG_MUTED.toUpperCase()
      },
      {
      ...
      const rule = domElement
        + '.' + 'ColorRef_' + colorRef + (d.tag !== '' ? '.' + d.tag : '') +
        ' { ' + cssElement + ': var(--' + d.color + '); }';
      sheet.insertRule(rule, sheet.cssRules.length);
      if (verbose) {
        console.log(rule);
      }
  ...
  private static generateUIColors(sheet: CSSStyleSheet, colorRef: ColorRefType, cssElement: string, domElement: string, verbose = false) {
    [
      {
        tag: '',
        color: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef
      },
      {
        tag: StaticDomTags.TAG_HOVER,
        color: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef + '_' + StaticDomTags.TAG_FOCUS.toUpperCase()
      },
      {
        tag: StaticDomTags.TAG_MUTED,
        color: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef + '_' + StaticDomTags.TAG_MUTED.toUpperCase()
      },
      {
        tag: StaticDomTags.TAG_FOCUS,
        color: StaticDomTags.TAG_UI_DIALOG + '_' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + colorRef + '_' + StaticDomTags.TAG_FOCUS.toUpperCase()
      ...
      const rule = domElement
        + '.' + StaticDomTags.TAG_UI_DIALOG + '_' + 'ColorRef_' + colorRef + (d.tag !== '' ? '.' + d.tag : '') +
        ' { ' + cssElement + ': var(--' + d.color + '); }';
      sheet.insertRule(rule, sheet.cssRules.length);
      if (verbose) {
        console.log(rule);
  ...
  generateColors() {
    this.netzgrafikColorStore.netzgrafikColors.forEach(netzgrafikColor => {
      ...
      document.documentElement.style.setProperty('--' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + netzgrafikColor.getColorRef(),
        colors.color);
      document.documentElement.style.setProperty('--' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + netzgrafikColor.getColorRef() +
        '_' + StaticDomTags.TAG_FOCUS.toUpperCase(),
        colors.colorFocus);
      document.documentElement.style.setProperty('--' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + netzgrafikColor.getColorRef() +
        '_' + StaticDomTags.TAG_MUTED.toUpperCase(),
        colors.colorMuted);
      document.documentElement.style.setProperty('--' + StaticDomTags.PREFIX_COLOR_VARIABLE + '_' + netzgrafikColor.getColorRef() +
        '_' + StaticDomTags.TAG_RELATED.toUpperCase(),
        colors.colorRelated);
  ...
  generateGlobalStyles(trainrunCategories: TrainrunCategory[],
    ...
    colorRefs.forEach(colorRef => this.generateColorAndStyling(colorRef, sheetKnotenauslastung, sheet, sheetLinePattern));

    return document.adoptedStyleSheets;
  }

  setNetzgrafikColors(netzgrafikColors: NetzgrafikColorDto[]) {
    this.netzgrafikColorStore.netzgrafikColors = netzgrafikColors.map(netzgrafikColor => new NetzgrafikColor(netzgrafikColor));
    this.netzgrafikColorUpdated();
  }

  getNetzgrafikColor(colorRef: ColorRefType): NetzgrafikColor {
    return this.netzgrafikColorStore.netzgrafikColors.find((netzgrafikColor: NetzgrafikColor) => netzgrafikColor.getColorRef() === colorRef);
  }

  changeColorRef(netzgrafikColorId: number, colorRef: ColorRefType) {
    const found = this.netzgrafikColorStore.netzgrafikColors.find(netzgrafikColor => netzgrafikColor.getId() === netzgrafikColorId);
    if (found !== undefined) {
  ...
  getDtos() {
    return this.netzgrafikColorStore.netzgrafikColors.map(netzgrafikColor => netzgrafikColor.getDto());
  }

  createDefaultColorForNotExistingColors(colorRef: ColorRefType, defaults: ColorRefType[] = ['EC', 'IC', 'IR', 'RE', 'S', 'G', 'GEX']) {
  ...
  private collectColorRefs(trainrunCategories: TrainrunCategory[],
                           trainrunSections: TrainrunSection[]): ColorRefType[] {
  ...
  private generateColorAndStyling(colorRef: ColorRefType,
    ...
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'stroke', StaticDomTags.EDGE_LINE_DOM_REF, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'stroke', StaticDomTags.EDGE_LINE_STOPS_DOM_REF, verbose);

    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'fill', StaticDomTags.EDGE_LINE_DOM_REF + '.' + StaticDomTags.TAG_HEADWAY_BAND, verbose);

    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'fill', StaticDomTags.EDGE_LINE_STOPS_DOM_REF + '.' + StaticDomTags.EDGE_LINE_STOPS_FILL, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'stroke', StaticDomTags.EDGE_LINE_STOPS_DOM_REF, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'fill', StaticDomTags.EDGE_LINE_STOPS_DOM_REF + '.' + StaticDomTags.TAG_FILL, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'stroke', StaticDomTags.EDGE_LINE_STOP_ICON_CLASS_DOM_REF, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'fill', StaticDomTags.EDGE_LINE_STOP_ICON_CLASS_DOM_REF + '.' + StaticDomTags.TAG_FILL, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'fill', StaticDomTags.EDGE_LINE_TEXT_DOM_REF, verbose);
    NetzgrafikColoringService.generateColors(sheet, colorRef,
      'stroke', StaticDomTags.TRANSITION_LINE_DOM_REF, verbose);

    // TODO - Knotenauslastung refactoring will impact this part
    NetzgrafikColoringService.generateUIColors(sheetKnotenauslastung, colorRef,
      'stroke', StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP_PATH, verbose);
    NetzgrafikColoringService.generateUIColors(sheetKnotenauslastung, colorRef,
      'fill', StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP_PATH, verbose);
    NetzgrafikColoringService.generateUIColors(sheetKnotenauslastung, colorRef,
      'fill', StaticDomTags.KNOTENAUSLASTUNG_DATA_GROUP_TEXT, verbose);
