...
export class CopyService implements OnDestroy {
  ...
  private filterSelectedTrainrun(copiedNetzgrafik: NetzgrafikDto) {
    ...
    if (selected !== null) {
      ...
      copiedNetzgrafik.trainruns = copiedNetzgrafik.trainruns.filter((t: TrainrunDto) => t.id === selected.getId());
      copiedNetzgrafik.trainrunSections = copiedNetzgrafik.trainrunSections.filter((ts: TrainrunSectionDto) => ts.trainrunId === selected.getId());

      copiedNetzgrafik.nodes = copiedNetzgrafik.nodes.filter((n: NodeDto) =>
        copiedNetzgrafik.trainrunSections.find((ts: TrainrunSectionDto) => ts.targetNodeId === n.id || ts.sourceNodeId === n.id) !== undefined
      );
    }
  }
  ...
  private filterTrainrun(copiedNetzgrafik: NetzgrafikDto) {
    ...
    trainrunToRemove.forEach((trainrunDto: TrainrunDto) => {
      copiedNetzgrafik.trainruns =
        copiedNetzgrafik.trainruns.filter((data: TrainrunDto) => data.id !== trainrunDto.id);
      copiedNetzgrafik.trainrunSections =
        copiedNetzgrafik.trainrunSections.filter((data: TrainrunSectionDto) => data.trainrunId !== trainrunDto.id);
    });
  }
  ...
  private filterTrainrunSection(copiedNetzgrafik: NetzgrafikDto) {
    ...
    copiedNetzgrafik.trainrunSections.forEach((trainrunSectionDto: TrainrunSectionDto) => {
      const trainrunSection: TrainrunSection = this.trainrunSectionService.getTrainrunSectionFromId(trainrunSectionDto.id);
    ...
    trainrunSectionToRemove.forEach((trainrunSectionDto: TrainrunSectionDto) => {
      copiedNetzgrafik.trainrunSections =
        copiedNetzgrafik.trainrunSections.filter((data: TrainrunSectionDto) => data.id !== trainrunSectionDto.id);
    });
  }

  private filterNote(copiedNetzgrafik: NetzgrafikDto) {
    copiedNetzgrafik.freeFloatingTexts = copiedNetzgrafik.freeFloatingTexts.filter((text: FreeFloatingTextDto) => {
  ...
  private filterMultiSelected(copiedNetzgrafik: NetzgrafikDto) {
    if (this.uiInteractionService.getEditorMode() === EditorMode.MultiNodeMoving) {
      ...
      copiedNetzgrafik.trainrunSections = copiedNetzgrafik.trainrunSections.filter((ts: TrainrunSectionDto) =>
  ...
  private removeEmptyTrainruns(copiedNetzgrafik: NetzgrafikDto) {
    copiedNetzgrafik.trainruns = copiedNetzgrafik.trainruns.filter((t: TrainrunDto) =>
      copiedNetzgrafik.trainrunSections.find((ts: TrainrunSectionDto) => ts.trainrunId === t.id) !== undefined
    );
  }
  ...
  private removeConnections(copiedNetzgrafik: NetzgrafikDto) {
    copiedNetzgrafik.nodes.forEach((n: NodeDto) => {
      n.connections = n.connections.filter((c: ConnectionDto) => {
        ...
        const pts1 = copiedNetzgrafik.trainrunSections.find((ts: TrainrunSectionDto) => port1.trainrunSectionId === ts.id);
        const pts2 = copiedNetzgrafik.trainrunSections.find((ts: TrainrunSectionDto) => port2.trainrunSectionId === ts.id);
  ...
  private saveCopiedDataToLocalStorage(netzgrafik: NetzgrafikDto) {
    try {
      localStorage.setItem(`CopyService`,
        JSON.stringify({
