...
export class PerlenketteNodeComponent implements OnInit {
  ...
  getConnectionIssue(): string {
    ...
    this.perlenketteNode.connections.forEach(mockConnection => mockConnection.connectionWarning && amountOfWarningConnections++);
  ...
  getFittingConnections(): string {
    ...
    this.perlenketteNode.connections.forEach(mockConnection => !mockConnection.connectionWarning && amountOfFittingConnections++);
  ...
  private calculateHeightConnectionSurplus() {
    const startEndNode = this.perlenketteNode.isTopNode(this.perlenketteTrainrun) ||
    ...
    if (this.isExpanded) {
      ...
      const bottomLen = this.perlenketteNode.getConnectionsBottomNodes(this.perlenketteTrainrun).length;
  ...
  getTextNameOrTime(connection: PerlenketteConnection, connectionGrpKey: number, pos: number) {
    return pos === 0 ? (connection.categoryShortName + '' + connection.title) : ('' + connection.remainingTime);
  }
  ...
  getColoringClassTag(connection: PerlenketteConnection = undefined): string {
    ...
    return ' ColorRef_' + trainrun2.getCategoryColorRef() +
      ' Freq_' + trainrun2.getFrequency() +
      ' LinePatternRef_' + trainrun2.getFrequencyLinePatternRef() +
      selected_tag;
  }

  getTextNameOrTimeClassTag(connection: PerlenketteConnection, connectionGrpKey: number, pos: number): string {
  ...
  getTextTerminalStationlassTag(connection: PerlenketteConnection, connectionGrpKey: number, pos: number): string {
  ...
  toggleNonStop() {
    ...
    if (transition !== undefined) {
      this.nodeService.toggleNonStop(this.perlenketteNode.nodeId, transition.getId());
  ...
  createPolygon(x: number, y: number): string {
    return 'M' + D3Utils.makeHexagonSVGPoints(new Vec2D(x, y), StaticDomTags.TRANSITION_BUTTON_SIZE) + 'z';
  }
    ...
    this.perlenketteTrainrun.pathItems.forEach((item: PerlenketteItem) => {
      if (item.isPerlenketteNode()) {
        item.getPerlenketteNode().connections.forEach((connection: PerlenketteConnection) => {
          ...
          maxTrainrunNameLen =
            Math.max(3 + connection.terminalStationBackward.length,
              Math.max(3 + connection.terminalStation.length,
